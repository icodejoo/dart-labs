...
/// [VmDanmakuConfig.enabled] is off or [VmDanmakuConfig.items] is empty.
///
/// Display-only (see [VmDanmakuConfig]'s doc comment): comments spawn as
/// playback position crosses their [VmDanmakuItem.time] and scroll right to
/// left over [VmDanmakuConfig.crossDuration], each assigned a horizontal
/// track round-robin to reduce overlap.
///
/// bilibili 风格滚动弹幕叠加层；[VmDanmakuConfig.enabled] 关闭或
/// [VmDanmakuConfig.items] 为空时不渲染任何内容。
///
/// 仅做展示（见 [VmDanmakuConfig] 的文档注释）：播放位置跨过弹幕的
/// [VmDanmakuItem.time] 时生成一条弹幕，在 [VmDanmakuConfig.crossDuration]
/// 内从右向左滚动划过，按轮询方式分配横向轨道以减少重叠。
class DanmakuTrackComponent extends VmComponent {
...
/// playback position crosses each [VmDanmakuItem.time].
///
/// 依据播放位置跨过每条 [VmDanmakuItem.time] 来生成/回收单条
...
/// last-seen playback position used to detect newly-crossed [VmDanmakuItem]s.
///
/// [_DanmakuTrack] 的状态；持有当前可见的弹幕，以及用于检测新跨过的
/// [VmDanmakuItem] 的最近播放位置。
class _DanmakuTrackState extends State<_DanmakuTrack> with VmPlugin<_DanmakuTrack> {
  ...
  /// Playback position as of the previous [VmProgress] tick.
  ///
  /// 上一次 [VmProgress] tick 时的播放位置。
  Duration _lastPosition = Duration.zero;

  /// Round-robin cursor over [VmDanmakuConfig.trackCount] horizontal tracks.
  ///
  ...
  /// behaves on scrub.
  ///
  /// 生成所有 [VmDanmakuItem.time] 落在 `(_lastPosition, position]` 区间内的
  ...
  void _spawn(VmDanmakuItem item) {
