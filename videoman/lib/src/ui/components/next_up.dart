...
///
/// Host-wired: the host constructs a [VmPlaylistController] and passes it in
/// (e.g. via a skin patch), since the running index lives on the controller,
/// not on [VmApi]. Visibility is driven off the throttled progress stream vs.
/// [VmState.duration] and [VmPlaylistConfig.nextUpLeadTime].
///
/// "下一集"卡片：当前项临近结束时淡入，提供一键跳到下一项。播放列表关闭、没有
/// 下一项、或宿主已对本项关闭卡片时，不渲染任何内容。
///
/// 由宿主接线：宿主构造一个 [VmPlaylistController] 传入（例如经皮肤补丁），因为
/// 运行时下标在控制器上、不在 [VmApi] 上。显隐由节流进度流对比
/// [VmState.duration] 与 [VmPlaylistConfig.nextUpLeadTime] 决定。
class NextUpComponent extends VmComponent {
  ...
  ///
  /// 该卡片读取并驱动的播放列表控制器。
  final VmPlaylistController controller;
...
class _NextUpViewState extends State<_NextUpView> with VmPlugin<_NextUpView> {
