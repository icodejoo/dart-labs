...
///
/// Host-wired: the host constructs a [VmAdController] and passes it in (e.g. via
/// a skin patch). Visibility follows the controller's phase; the skip countdown
/// follows the throttled progress stream against [VmAdBreak.skippableAfter].
///
/// 广告播放时显示的叠层："广告"角标、广告超过可跳过阈值后出现的跳过控件，以及
/// 覆盖整个画面、上报点击跳转的点按（打开 URL 由宿主而非库负责）。广告关闭或当前
/// 没有广告在播时不渲染任何内容。
///
/// 由宿主接线：宿主构造一个 [VmAdController] 并传入（例如经皮肤补丁）。显隐跟随
/// 控制器阶段；跳过倒计时依节流进度流对比 [VmAdBreak.skippableAfter]。
class AdOverlayComponent extends VmComponent {
  ...
  ///
  /// 该叠层读取并驱动的广告控制器。
  final VmAdController controller;
...
/// 推导跳过倒计时。
class _AdOverlayViewState extends State<_AdOverlayView> with VmPlugin<_AdOverlayView> {
