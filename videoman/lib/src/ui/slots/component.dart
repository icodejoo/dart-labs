...
/// [VmPatch] targeting, a [slot] deciding which region of the overlay it
...
/// never see raw [VmComponent] children.
///
/// 播放器组件树中的可寻址节点。
///
/// 每个节点都有稳定的 [name]，用于拼接 [VmPatch] 目标路径；[slot] 决定它
/// 渲染到叠加层的哪个区域；[order] 用于同槽位内的稳定排序；[children] 是
/// 组合节点（例如控制条分组）的可选子节点。[build] 收到的子节点已构建为
/// [Widget]——组合组件永远不会看到原始的 [VmComponent] 子节点。
abstract class VmComponent {
  /// Stable identifier used for path matching in [VmPatch] operations.
  ///
  /// 用于 [VmPatch] 操作路径匹配的稳定标识符。
  String get name;
  ...
  List<VmComponent> get children => const [];
