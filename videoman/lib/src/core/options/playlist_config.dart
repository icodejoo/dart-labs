...
/// `VmPlaylistController` (host-constructed), keeping this bundle immutable and
/// comparable. Disabled and empty by default so enabling it is an explicit
/// host decision.
///
/// 顺序播放列表（剧集/曲目）与临近结束显示的"下一集"卡片的配置。
///
/// 刻意只做数据：运行时下标与导航在 `VmPlaylistController`（宿主构造）上，
/// 使本配置保持不可变、可比较。默认关闭且列表为空，是否启用由宿主显式决定。
class VmPlaylistConfig {
  ...
  ///
  /// 要顺序播放的项列表。
  final List<VmPlaylistItem> items;
  ...
  const VmPlaylistConfig({
    this.enabled = false,
    this.items = const <VmPlaylistItem>[],
    this.initialIndex = 0,
  ...
  VmPlaylistConfig copyWith({
    bool? enabled,
    List<VmPlaylistItem>? items,
    int? initialIndex,
    bool? autoPlayNext,
    Duration? nextUpLeadTime,
  }) {
    return VmPlaylistConfig(
