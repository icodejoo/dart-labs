...
/// Drives sequential playlist playback on top of a [VmApi]: tracks the current
/// index, navigates between items by calling [VmApi.open], and (optionally)
...
/// [VmApi]. Seed its items/index/auto-advance from [VmOptions.playlist]; the
/// host still performs the very first [VmApi.open] (or calls [jumpTo]).
///
/// 在 [VmApi] 之上驱动顺序播放列表：跟踪当前下标、通过调用 [VmApi.open] 在项间
/// 导航，并（可选）在一项播完时自动前进。
///
/// 纯 Dart（无 Flutter 依赖），因此可单测并归属核心层。由宿主构造并注入——engine
/// 从不持有它，"下一集"卡片直接读该控制器而非经 [VmApi]。其项/下标/自动续播取自
/// [VmOptions.playlist]；首次 [VmApi.open]（或调用 [jumpTo]）仍由宿主执行。
///
/// **Composing with ads:** both this controller and a `VmAdController` react to
/// [VmCompleted]; do not enable both against one player at once. Set
/// [VmPlaylistConfig.autoPlayNext] to `false` and drive [next] from
/// `VmAdController.contentEnded` instead so ads and content-advance don't race.
///
/// **与广告组合：** 本控制器与 `VmAdController` 都响应 [VmCompleted]；不要在同一
/// 播放器上同时启用二者的自动行为。应把 [VmPlaylistConfig.autoPlayNext] 设为
/// `false`，改由 `VmAdController.contentEnded` 驱动 [next]，避免广告与换集争抢。
class VmPlaylistController {
  /// Creates a controller bound to [api], seeded from [VmOptions.playlist].
  ///
  /// 创建绑定到 [api] 的控制器，初值取自 [VmOptions.playlist]。
  ///
  /// - [api]: the player capability surface to drive / 要驱动的播放器能力面
  ///
  /// Example / 示例:
  /// ```dart
  /// final playlist = VmPlaylistController(api);
  /// await playlist.jumpTo(0); // open the first item
  /// ```
  VmPlaylistController(this._api)
  ...
  /// failed [VmApi.open] leaves the current index unchanged rather than
  /// advancing to an item that never started.
  ///
  /// 跳转到 [index] 并打开其源；越界下标被忽略。
  ///
  /// 先打开、再提交下标并通知 [indexChanges]，使 [VmApi.open] 失败时当前下标
  /// 保持不变，而非前进到一个从未起播的项。
  ///
  ...
  void _onEvent(VmEvent e) {
    if (e is VmCompleted && _autoPlayNext && hasNext) {
      unawaited(next());
    }
  }
