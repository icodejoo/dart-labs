...
/// Returns the first not-yet-played mid-roll break whose [VmAdBreak.offset] has
...
/// following one.
///
/// 返回首个尚未播放、且 [VmAdBreak.offset] 在 [position] 处已到达的中插广告位；
/// 没有到期的则返回 null。
///
/// 纯函数，使中插触发规则可单独单测。最早到期者优先；调用方将其标记为已播，
/// 下一个 tick 便浮现下一条。
VmAdBreak? dueMidRoll(
  List<VmAdBreak> breaks,
  Duration position,
  Set<VmAdBreak> played,
) {
  for (final b in breaks) {
    if (b.kind == VmAdBreakKind.mid &&
        !played.contains(b) &&
        b.offset <= position) {
      return b;
    }
...
/// Orchestrates pre/mid/post-roll ads on top of a [VmApi] by swapping the
...
/// [VmApi.open] to start content with its ads. Mid-roll saves the content
...
/// [VmAdConfig.onAdEvent].
///
/// **Composing with a `VmPlaylistController`:** both react to [VmCompleted], so
...
///
/// 在 [VmApi] 之上编排前/中/后贴片广告：在正片与各广告位之间切换正在播放的源。
///
/// 纯 Dart（无 Flutter 依赖），可单测并归属核心层。由宿主构造并注入——用 [load]
/// 代替 [VmApi.open] 来带广告地起播正片。中插会保存正片位置并在广告后从该处续播
/// （依赖 engine 的 seek 寄存，使续播位置在被续播源尚未报告时长前也能落地）。
/// 点击跳转不会在此处执行——经 [VmAdConfig.onAdEvent] 暴露给宿主。
///
/// **与 `VmPlaylistController` 组合：** 二者都响应 [VmCompleted]，裸挂到同一播放器
/// 会争抢 `open()`。组合时应把播放列表的 `autoPlayNext` 设为 `false`，改由本控制器的
/// [contentEnded]（只在正片**及其**后贴片都播完后才触发）来驱动换集。
class VmAdController {
  /// Creates a controller bound to [api], seeded from [VmOptions.ads].
  ///
  /// 创建绑定到 [api] 的控制器，初值取自 [VmOptions.ads]。
  ///
  /// - [api]: the player capability surface to drive / 要驱动的播放器能力面
  ///
  /// Example / 示例:
  /// ```dart
  /// final ads = VmAdController(api);
  /// await ads.load(const VmSource('https://host/movie.m3u8'));
  /// ```
  VmAdController(this._api)
  ...
  final VmApi _api;
  final List<VmAdBreak> _breaks;
  final void Function(VmAdEvent)? _onAdEvent;
  final bool _enabled;
  StreamSubscription<VmEvent>? _eventSub;
  ...
  VmAdBreak? _current;
  final Set<VmAdBreak> _played = <VmAdBreak>{};
  ...
  ///
  /// 当前正在播放的广告位；无则为 null。
  VmAdBreak? get currentBreak => _current;
  ...
  /// `VmPlaylistController` (with its own `autoPlayNext` off) without racing on
  /// [VmCompleted].
  ///
  /// 在正片彻底结束后触发一次——即正片播完且所有后贴片也播完之后。用作在不与
  /// [VmCompleted] 争抢的前提下推进 `VmPlaylistController`（其 `autoPlayNext` 关闭）
  /// 的组合接缝。
  ...
  Future<void> load(VmSource content) async {
    _content = content;
    _played.clear();
    _contentResumeAt = Duration.zero;
    _lastContentPosition = Duration.zero;
    if (_enabled) {
      final pre = _firstOfKind(VmAdBreakKind.pre);
      if (pre != null) {
        await _playAd(pre);
        return;
      }
  ...
  /// runtime rather than pre-configured in [VmAdConfig.breaks]; only fires
  /// while content is actually playing. [ad] should be a
  /// [VmAdBreakKind.mid] break — its own [VmAdBreak.offset] is ignored, "now"
  /// is the current content position.
  ///
  /// 立即打断正片，在任意时点插播 [ad]，播完回到打断处续播。用于运行时（而非
  /// 在 [VmAdConfig.breaks] 预配置）插播的广告；仅在正片确实在播放时生效。[ad]
  /// 应为 [VmAdBreakKind.mid]——其自身的 [VmAdBreak.offset] 被忽略，"当下"即当前
  /// 正片位置。
  ///
  ...
  /// Example / 示例:
  /// ```dart
  /// ads.playAdNow(const VmAdBreak(
  ///   kind: VmAdBreakKind.mid,
  ///   source: VmSource('https://host/flash-sale.mp4'),
  ///   skippableAfter: Duration(seconds: 5),
  /// ));
  /// ```
  Future<void> playAdNow(VmAdBreak ad) async {
  ...
  void skip() {
    ...
    final b = _current!;
    _fire(VmAdEventType.skipped, b);
    unawaited(_resumeAfterAd(b));
  }

  /// Reports a click on the current ad; the host acts on
  /// [VmAdBreak.clickThroughUrl], the library does not navigate.
  ///
  /// 上报一次对当前广告的点击；由宿主处理 [VmAdBreak.clickThroughUrl]，库不跳转。
  void notifyClicked() {
    final b = _current;
    if (b != null) _fire(VmAdEventType.clicked, b);
  }

  /// Returns the first not-yet-played break of [kind], or null.
  ///
  /// 返回首个尚未播放的、类型为 [kind] 的广告位；没有则为 null。
  VmAdBreak? _firstOfKind(VmAdBreakKind kind) {
  ...
  ///
  /// 把播放切换到广告位 [b]。
  Future<void> _playAd(VmAdBreak b) async {
  ...
  void _onEvent(VmEvent e) {
    if (e is! VmCompleted) return;
    if (_phase == _Phase.ad) {
      final finished = _current;
      if (finished == null) return;
      _fire(VmAdEventType.completed, finished);
      unawaited(_resumeAfterAd(finished));
    } else if (_phase == _Phase.content) {
      final post = _enabled ? _firstOfKind(VmAdBreakKind.post) : null;
      if (post != null) {
  ...
  void _fire(VmAdEventType type, VmAdBreak b) => _onAdEvent?.call(VmAdEvent(type, b));
