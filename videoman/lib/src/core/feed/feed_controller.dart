...
/// Drives a [VmFeedEnginePool] through a vertical feed of [VmFeedItem]s
...
/// [VmFeedEnginePool.bind]-equivalent `open()` calls) — see doc/SPEC.md's
...
/// (`VmFeedPlayer`) owns the `PageView` and the pool's lifetime, calls
/// [activate] on page settle, and [slotFor]/[peek] to render each page.
///
/// 用一个 [VmFeedEnginePool] 驱动纵向 feed 的 [VmFeedItem] 列表
...
/// （`VmFeedPlayer`）持有 `PageView` 与池的生命周期，在翻页定格时调用
...
class VmFeedController {
  ...
  VmFeedSlot? slotFor(int index) => pool.slotFor(index);
  ...
  /// each call [VmFeedEnginePool.retain] with their own window and then race
  ...
  /// [VmFeedEnginePool.retain]，再争抢空闲引擎，落败的一方可能把胜出方刚建立
  ...
  Future<void> _runActivateLoop() async {
    try {
      while (_pendingIndex != null) {
        ...
        final window = vmFeedWindow(index, pool.size);
