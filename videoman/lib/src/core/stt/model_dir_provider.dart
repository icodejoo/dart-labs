...
///
/// Kept as a port for the same reason as `VmThumbDirProvider`: the default
...
///
/// 做成端口的原因与 `VmThumbDirProvider` 相同：默认答案需要 `path_provider`
/// （`getApplicationSupportDirectory()`）——它是 Flutter 插件，
/// `lib/src/core/**` 下不允许引入。具体实现放在
/// `lib/src/platform_impl/stt_model_dir_impl.dart`。
abstract class VmSttModelDirProvider {
...
/// A [VmSttModelDirProvider] that always returns one fixed path.
///
/// Lets hosts pin the model cache location and keeps tests free of plugin
/// channels.
///
/// 恒定返回同一路径的 [VmSttModelDirProvider]。
///
/// 既支持宿主固定模型缓存位置，也让测试无需依赖插件通道。
class FixedSttModelDirProvider implements VmSttModelDirProvider {
