...
///
/// Produced by a [VmSttEngine] as it transcribes; the UI overlay renders
/// whichever cue's `[start, end)` range contains the current playback
/// position.
///
/// 一条识别出的文本区间，附带其覆盖的媒体位置范围。
///
/// 由 [VmSttEngine] 边转写边产出；UI 叠层渲染当前播放位置落在
/// `[start, end)` 区间内的那一条。
///
/// Example / 示例:
/// ```dart
...
class VmSttCue {
  ...
  ///
  /// 创建一条字幕。
  const VmSttCue({required this.text, required this.start, required this.end});
  ...
  bool operator ==(Object other) =>
      ...
      other is VmSttCue && text == other.text && start == other.start && end == other.end;
