...
/// cached in.
///
/// A distinct port from [VmSttModelDirProvider] even though both resolve to
...
///
/// 与 [VmSttModelDirProvider] 是两个独立的端口，即便两者默认都落在类似的
...
abstract class VmSttSubtitleDirProvider {
...
/// A [VmSttSubtitleDirProvider] that always returns one fixed path.
///
/// Lets hosts pin the subtitle cache location and keeps tests free of plugin
/// channels.
///
/// 恒定返回同一路径的 [VmSttSubtitleDirProvider]。
///
/// 既支持宿主固定字幕缓存位置，也让测试无需依赖插件通道。
class FixedSttSubtitleDirProvider implements VmSttSubtitleDirProvider {
