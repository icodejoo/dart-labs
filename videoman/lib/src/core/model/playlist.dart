...
/// 卡片展示的可选元数据。
class VmPlaylistItem {
  /// The playable source for this entry.
  ///
  /// 该项对应的可播放源。
  final VmSource source;

  /// Card title; falls back to [VmSource.title] when omitted.
  ///
  /// 卡片标题；省略时回退到 [VmSource.title]。
  final String? title;
  ...
  /// Example / 示例:
  /// ```dart
  /// const VmPlaylistItem(
  ///   source: VmSource('https://host/ep3.m3u8'),
  ///   title: '第三集',
  /// );
  /// ```
  const VmPlaylistItem({
