...
/// (e.g. call an API) — videoman never calls a backend itself and never
/// rolls the local toggle back regardless of what the host does with it.
///
/// 点赞本地状态切换后的回调，供宿主自行持久化（例如调用 API）——videoman
/// 自身从不调用后端，也不会因宿主后续行为回滚本地已切换的状态。
///
/// - [liked]: the new local liked flag / 切换后的本地点赞状态
/// - [likeCount]: the new local like count / 切换后的本地点赞数
typedef VmFeedLikeChanged = void Function(bool liked, int likeCount);

/// One playable entry in a [VmFeedController]-driven vertical feed
/// (bilibili/douyin-style "swipe for next video").
///
/// Carries the playable [source] plus the social-chrome metadata
/// [VmDouyinSkin] renders (author/music/counts) and the callbacks fired for
/// actions videoman does not own the business logic for (comment/share/
/// avatar/follow) — see doc/PRD.md ADR: videoman owns like state locally
/// end-to-end (toggle + count + heart animation), everything else is a
/// passive callback.
///
/// [VmFeedController] 驱动的纵向 feed（bilibili/douyin 式"上滑下一个视频"）
/// 中的一条条目。
///
/// 除可播放的 [source] 外，还带 `VmDouyinSkin` 渲染所需的社交 chrome 元数据
/// （作者/音乐/计数），以及 videoman 不持有其业务逻辑的动作回调（评论/分享/
/// 头像/关注）——见 doc/PRD.md ADR：点赞状态由 videoman 端到端本地持有
...
class VmFeedItem {
  ...
  /// for videoman's local like state (see [VmFeedController.toggleLike]).
  ///
  /// 当前观众是否已点赞该条目；作为 videoman 本地点赞状态的初值（见
  /// [VmFeedController.toggleLike]）。
  ...
  /// Comment count shown on the social rail; display-only, videoman never
  /// mutates it (tapping comment only fires [onComment]).
  ///
  /// 社交竖排展示的评论数；仅展示，videoman 从不修改它（点击评论只触发
  /// [onComment]）。
  ...
  /// button); videoman calls this after already updating its own local state.
  ///
  /// 本地点赞状态变化时触发（双击或点赞按钮）；videoman 会先更新自身本地
  /// 状态，再调用该回调。
  final VmFeedLikeChanged? onLikeChanged;

  /// Fired when the comment affordance is tapped; videoman owns no comment UI.
  ///
  /// 点击评论入口时触发；videoman 不持有任何评论 UI。
  final void Function()? onComment;

  /// Fired when the share affordance is tapped; videoman owns no share UI.
  ///
  /// 点击分享入口时触发；videoman 不持有任何分享 UI。
  ...
  /// Fired when the follow affordance is tapped; videoman owns no follow
  /// state (whether the viewer already follows the author is the host's).
  ///
  /// 点击关注入口时触发；videoman 不持有关注状态（观众是否已关注作者由
  /// 宿主自行维护）。
  ...
  /// [VmFeedController] to persist a like toggle into its item cache.
  ///
  /// 返回一份替换了指定字段的拷贝；供 [VmFeedController] 内部把点赞切换结果
  /// 落回条目缓存。
  VmFeedItem copyWith({bool? initialLiked, int? initialLikeCount}) {
    return VmFeedItem(
      source: source,
