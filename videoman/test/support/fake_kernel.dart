...
/// A test double for [VmKernel] that records every call it receives and lets
...
///
/// [VmKernel] 的测试替身：记录收到的每次调用，并允许测试向其流中主动推送状态。
///
/// 在整个测试套件中（本任务及后续任务）作为唯一共享的假内核实现。
class FakeKernel implements VmKernel {
  ...
  final StreamController<VmSize> _size = StreamController<VmSize>.broadcast();
  ...
  @override
  Stream<VmSize> get size => _size.stream;
  ...
  ///
  /// 向 [size] 推送一个视频帧尺寸。
  void emitSize(int width, int height) => _size.add(VmSize(width: width, height: height));
