...
import 'package:videoman/src/core/stt/model_dir_provider.dart';
import 'package:videoman/src/core/stt/model_provider.dart';
import 'package:videoman/src/core/stt/model_spec.dart';
...
void main() {
  ...
  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('videoman_stt_model_test_');
  });
  ...
  VmSttModelSpec buildSpec() {
    return VmSttModelSpec(
  ...
  test('downloads every missing file and resolves their local paths', () async {
    ...
    final downloader = VmSttModelDownloader(
  ...
  test('emits one progress step per file in order', () async {
    ...
    final steps = <VmSttModelProgress>[];
  ...
  test('throws VmSttModelDownloadException when a file cannot be fetched', () async {
    ...
    await expectLater(
      ...
      throwsA(isA<VmSttModelDownloadException>()),
    );
  });
