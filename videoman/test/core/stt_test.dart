...
void main() {
  group('VmSttCue', () {
    test('covers is inclusive of start and exclusive of end', () {
      const cue = VmSttCue(text: 'hi', start: Duration(seconds: 1), end: Duration(seconds: 2));
    ...
    test('equality is value-based', () {
      const a = VmSttCue(text: 'hi', start: Duration.zero, end: Duration(seconds: 1));
      const b = VmSttCue(text: 'hi', start: Duration.zero, end: Duration(seconds: 1));
      const c = VmSttCue(text: 'bye', start: Duration.zero, end: Duration(seconds: 1));
      expect(a, b);
      expect(a, isNot(c));
    });
  });
  ...
  group('VmSttConfig', () {
    ...
    test('equality compares engine by identity', () {
      final engine = NoopSttEngine();
      final a = VmSttConfig(enabled: true, engine: engine);
      final b = VmSttConfig(enabled: true, engine: engine);
      final c = VmSttConfig(enabled: true, engine: NoopSttEngine());
      expect(a, b);
      expect(a, isNot(c));
    });
  });
  ...
  group('VmOptions', () {
    test('carries a default VmSttConfig and includes it in equality', () {
      const options = VmOptions();
      expect(options.stt, const VmSttConfig());

      final withStt = options.copyWith(stt: const VmSttConfig(enabled: true));
