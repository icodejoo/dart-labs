...
/// Hands out [FakeVmApi] instances as a [VmEngineFactory] would and keeps
/// every one it created, so tests can assert on how many engines a pool
/// allocated and what happened to each.
///
/// 像 [VmEngineFactory] 一样发放 [FakeVmApi] 实例，并保留创建过的每一个，
...
void main() {
  group('vmFeedWindow', () {
    test('expands outward from the centre, forward first', () {
      expect(vmFeedWindow(5, 1), [5]);
      expect(vmFeedWindow(5, 2), [5, 6]);
      expect(vmFeedWindow(5, 3), [5, 6, 4]);
      expect(vmFeedWindow(5, 4), [5, 6, 4, 7]);
      expect(vmFeedWindow(5, 5), [5, 6, 4, 7, 3]);
    });
    ...
    test('skips negative indices near the top of the feed instead of clamping '
      ...
      expect(vmFeedWindow(0, 3), [0, 1, 2]);
      expect(vmFeedWindow(1, 4), [1, 2, 0, 3]);
    });
  });

  group('VmFeedEnginePool', () {
    test('never allocates more engines than size, no matter how many indices '
        'are bound', () async {
      final fleet = _Fleet();
      final pool = VmFeedEnginePool(engineFactory: fleet.make, size: 2);

      for (var i = 0; i < 6; i++) {
        await pool.bind(i, VmSource('https://h/$i.mp4'));
      }
    ...
    test('applies fit once to each engine as it is created', () async {
      final fleet = _Fleet();
      final pool = VmFeedEnginePool(engineFactory: fleet.make, size: 2, fit: VmFit.cover);

      await pool.bind(0, const VmSource('https://h/0.mp4'));
      await pool.bind(1, const VmSource('https://h/1.mp4'));

      expect(fleet.created.map((a) => a.lastFit), [VmFit.cover, VmFit.cover]);
      await pool.dispose();
    });
    ...
    test('bind opens the source and marks the slot ready only once open resolves', () async {
      ...
      final binding = pool.bind(0, const VmSource('https://h/0.mp4'), autoPlay: true);
    ...
    test('rebinding the same index to the same uri reuses the binding instead '
      ...
      final fleet = _Fleet();
      final pool = VmFeedEnginePool(engineFactory: fleet.make, size: 3);

      await pool.bind(0, const VmSource('https://h/0.mp4'));
      await pool.bind(0, const VmSource('https://h/0.mp4'), autoPlay: true);
    ...
    test('retain unbinds indices outside the window, pausing their engines but '
      ...
      await pool.bind(0, const VmSource('https://h/0.mp4'));
      await pool.bind(1, const VmSource('https://h/1.mp4'));
      await pool.bind(2, const VmSource('https://h/2.mp4'));
    ...
    test('a released engine is reused for the next bind rather than a new one '
      ...
      await pool.bind(9, const VmSource('https://h/9.mp4'));
    ...
    test('a binding superseded while its open() was in flight never flips to '
      ...
      final slow = pool.bind(0, const VmSource('https://h/0.mp4'));
      pool.retain(const <int>[]);
      await slow;
    ...
    test('size is clamped to at least 1 so a bogus 0 cannot deadlock binding', () async {
      final fleet = _Fleet();
      final pool = VmFeedEnginePool(engineFactory: fleet.make, size: 0);
