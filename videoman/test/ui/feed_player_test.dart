...
/// A [FakeVmApi] whose [open] takes [openDelay] before resolving — used to
/// hold a feed page in its not-yet-ready state long enough to assert on the
/// placeholder.
///
/// 一个 [open] 会等待 [openDelay] 才 resolve 的 [FakeVmApi]——用于把某页 feed
/// 停在"尚未就绪"状态足够久，以便对占位进行断言。
class _SlowOpenApi extends FakeVmApi {
...
/// Hands out fake engines to [VmFeedPlayer] and keeps every one, so tests can
/// assert on how many the pool built and what it did with them.
///
/// 向 [VmFeedPlayer] 发放假引擎并保留每一个，供测试断言池建了多少个引擎、
/// 对它们做了什么。
class _Fleet {
...
void main() {
  testWidgets('defaults to cover fit so switching items never letterbox-jumps', (t) async {
    ...
    expect(fleet.created.every((a) => a.lastFit == VmFit.cover), isTrue);
  });
  ...
  testWidgets('honors an explicit fit override', (t) async {
    ...
    expect(fleet.created.first.lastFit, VmFit.contain);
  });
  ...
  testWidgets('activates the first page on mount', (t) async {
    ...
    await t.pumpWidget(MaterialApp(
      home: VmFeedPlayer(
        engineFactory: fleet.make,
        loader: (i) async =>
            VmFeedItem(source: VmSource('https://h/$i.mp4'), authorName: 'author$i'),
      ),
    ));
    await t.pumpAndSettle();
  ...
  testWidgets('an ended feed (loader returns null) shows a black placeholder '
    ...
    final fleet = _Fleet();
    await t.pumpWidget(MaterialApp(
      home: VmFeedPlayer(engineFactory: fleet.make, loader: (i) async => null),
    ));
    await t.pumpAndSettle();
