...
import 'package:videoman/src/ui/components/next_up.dart';
...
void main() {
  ...
  (FakeVmApi, VmPlaylistController) build({bool enabled = true}) {
    final api = FakeVmApi(
      options: VmOptions(
        playlist: VmPlaylistConfig(enabled: enabled, items: _items),
      ),
    );
  ...
  Future<void> tick(
    ...
    api.pushProgress(VmProgress(position: position));
    await t.pump();
    await t.pump();
  }
