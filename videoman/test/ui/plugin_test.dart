...
import 'package:videoman/src/ui/scope/plugin.dart';
import 'package:videoman/src/ui/scope/scope.dart';

import '../support/fake_api.dart';

/// A minimal [VmPlugin] host: binds to [stream] in initState and records
/// every event, and captures the resolved [api] for assertions.
///
/// 最小 [VmPlugin] 宿主：在 initState 绑定 [stream] 并记录每个事件，同时
/// 捕获解析出的 [api] 供断言。
...
class _HostState extends State<_Host> with VmPlugin<_Host> {
  VmApi? seenApi;
  @override
  void initState() {
    super.initState();
    seenApi = api; // resolves via VmScope.readOf — safe in initState
