...
import 'package:videoman/src/ui/components/subtitle.dart';
...
void main() {
  ...
  group('SubtitleOverlayComponent', () {
    ...
    testWidgets('shows the cue text once one is pushed', (t) async {
      ...
      api.stt.push(const VmSttCue(text: '大家好', start: Duration.zero, end: Duration(seconds: 5)));
      await t.pump();
      await t.pump();
    ...
    testWidgets('hides again once a progress tick moves past the cue end', (t) async {
      ...
      api.stt.push(const VmSttCue(text: '大家好', start: Duration.zero, end: Duration(seconds: 2)));
      await t.pump();
      await t.pump();
      expect(find.text('大家好'), findsOneWidget);
      ...
      api.stt.clear();
      api.pushProgress(const VmProgress(position: Duration(seconds: 5), buffer: Duration.zero));
      await t.pump();
      await t.pump();
