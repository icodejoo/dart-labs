...
import 'package:videoman/src/ui/components/hud_layer.dart';
...
void main() {
  testWidgets('volume HUD shows a speaker icon and NN%', (t) async {
    final api = FakeVmApi();
    api.push(const VmState(volume: 30));
    api.pushUi(const VmUiState(hud: VmHud.volume));
  ...
  testWidgets('volume HUD shows the muted icon at zero', (t) async {
    final api = FakeVmApi();
    api.push(const VmState(volume: 0));
    api.pushUi(const VmUiState(hud: VmHud.volume));
    await pumpComponent(t, api, HudLayerComponent());
    await t.pump();
  ...
  testWidgets('brightness HUD shows a brightness icon and NN%', (t) async {
    final api = FakeVmApi();
    api.push(const VmState(brightness: 0.4));
    api.pushUi(const VmUiState(hud: VmHud.brightness));
    await pumpComponent(t, api, HudLayerComponent());
    await t.pump();
