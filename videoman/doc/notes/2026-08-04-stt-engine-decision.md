...
4. 模型分发：**默认 GitHub Releases 源，宿主可自定义源**（已定案，见下）；多文件模型
   （Zipformer 是 encoder+decoder+joiner+tokens 四个文件）需要 `VmSttModelProvider`
   逐文件下载校验，不是单文件下载就完事。
...
- **⚠️ 2026-08-04 修正**：sherpa-onnx 官方 GitHub Releases 发布的是
  ...
  裸文件重新托管**（自己的 GitHub Release 或 CDN），`VmSttModelProvider` 只管逐个
  下载裸文件，不做任何解压。也就是说"默认 GitHub Releases 源"实际指
  **videoman 自己重新托管的 Release**，不是 `k2-fsa/sherpa-onnx` 官方仓库的
  Release（那边下的是压缩包）。**待办**：把 zh-en Zipformer 的四个文件解压后
  重新上传到 videoman 自己的 GitHub Release，产出实际下载地址。
- 复用阶段 B 预览缩略图的下载/缓存基础设施模式，**已实现**：
  `lib/src/core/stt/model_spec.dart`（`VmSttModelFile`/`VmSttModelSpec`/
  `VmSttModelFiles`，每个文件独立 URL + 可选 `sizeBytes`/`sha256`）、
  `lib/src/core/stt/model_dir_provider.dart`（`VmSttModelDirProvider` 端口，
  镜像 `VmThumbDirProvider`）、`lib/src/core/stt/model_provider.dart`
  （`VmSttModelProvider`/`VmSttModelDownloader`：按模型 id 分子目录、按已缓存
  文件大小跳过重复下载、逐文件进度流、任一文件失败即抛
  `VmSttModelDownloadException`——**不像预览缓存那样静默降级**，因为缺一个模型
  ...
  （同 `VmDiskThumbCache` 的做法）尚未实现**——当前 `VmSttModelDownloader` 只有
...
- **拆成独立包 `videoman_stt`**（monorepo 内，`dart-labs/videoman_stt/`）——原因：
  ...
  `videoman` 主包，所有用 videoman 的 App 都会背上这部分体积，不管有没有用
  字幕功能。`videoman` 主包**零依赖 `sherpa_onnx`**，只暴露 `VmSttConfig.engine`
  抽象接口；`videoman_stt` 依赖 `videoman`（monorepo 内 path 依赖）+
  `sherpa_onnx`，实现具体的 `ZipformerSttEngine`。想要字幕功能的宿主自己额外加
  一个 `videoman_stt` 依赖，不需要的宿主完全不受影响。
- **接口补了一个漏洞**：`VmSttEngine.start()` 原来不带参数——但引擎自己并不
  知道 `feed()` 喂进来的音频对应播放器哪个时间点，没法给 `VmSttCue` 打对
  时间戳。改成 `start(Duration atPosition)`，引擎从这个基准点按累计采样数
  （采样数/采样率）推算每条字幕的 `start`/`end`。`VmSttService` 已同步改造
  （用它本来就在维护的 `_position`）。
- `ZipformerSttEngine`（`videoman_stt/lib/src/zipformer_engine.dart`）：包住
...
- **⚠️ 全程未跑过真机/真实设备验证**：`flutter pub get`/`flutter analyze` 在
  `videoman_stt` 上跑通了、依赖能正常解析，`sherpa_onnx` 包源码里的 API 签名
  也逐字核对过，但**从未真正构建过 APK/IPA、从未在真实音频流上跑过一次
  `ZipformerSttEngine`**——与 videoman 其余"真机未验证"事项同一状态。
- **模型分发地址仍是占位**：`videoman_stt/lib/src/model_specs.dart` 里
...
- `SubtitleOverlayComponent`（`VmSlot.overlay`）：渲染覆盖当前播放位置的字幕；
...
- `SubtitleButtonComponent`（`TopBarComponent` 的子组件）：同样在无引擎时不
  ...
  （`VmStrings.subtitleOff`，新增字段，避免组件里出现字面量中文）。是否已
  开启是组件本地状态（`VmApi.stt` 没有暴露"是否在运行"的字段），后续如果
  要在多处入口保持同步，需要把这个状态提升到 `VmState`。
...
- `core/stt/subtitle_dir_provider.dart` + `platform_impl/stt_subtitle_dir_impl.dart`：
  `VmSttSubtitleDirProvider`/`TempSttSubtitleDirProvider`——与
  `VmSttModelDirProvider` 是**两个独立端口**（虽然默认都落在应用支持目录下的
  子文件夹），因为缓存的东西生命周期不同：字幕文件按来源 key、模型文件按
  模型 id key，宿主可能想分别清理。
- `core/stt/subtitle_store.dart`：`VmSttSubtitleStore`/`VmFileSttSubtitleStore`——
  按来源 URI 的 `fnv1a64` 哈希命名 `.srt` 文件（复用预览缓存同一套 key 命名
  约定），损坏文件降级为"未缓存"而非崩溃。7 个测试。
- `videoman_stt`：`ZipformerSttEngine.transcribeWavFile(wavPath)`——复用引擎
