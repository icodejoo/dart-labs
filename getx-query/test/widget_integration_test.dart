...
class _SlowCounterVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    counter = this.useQuery(queryKey: ['slow-counter'], queryFn: (_) => _future);
  }
}
...
class _DynamicVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    result = this.useQuery(queryKey: ['dynamic'], queryFn: (_) => _fn());
  }
}
...
class _CacheVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    items = this.useQuery(queryKey: ['cache-items'], queryFn: (_) async => 'original');
  }
...
class _RxKeyVM extends GetBaseViewModel {
  ...
  void onInit() {
    ...
    content = this.useQuery(
      queryKey: ['content', tab],
      queryFn: (ctx) async => 'data-${ctx.queryKey.last}',
    );
  }
}
...
class _MutateVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    items = this.useQuery(queryKey: ['mut-items'], queryFn: (_) async => serverValue);
  }
...
class _Page2VM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    data = this.useQuery(queryKey: ['page2-data'], queryFn: (_) async => 'hello');
  }
...
void main() {
  ...
  testWidgets('Global useQuery (no ViewModel) drives Obx correctly', (tester) async {
    ...
    final result = useQuery(
      queryKey: ['global-widget'],
      queryFn: (_) async => 'global-value',
      client: client,
    );
  ...
  // 9. QueryService resolves via useQueryClient
  testWidgets('QueryService registered: useQuery resolves without explicit client', (tester) async {
    ...
    final result = useQuery(
      queryKey: ['service-widget'],
      queryFn: (_) async => 'from-service',
  ...
  // 10. useMutation in widget: isPending -> isSuccess -> Obx updates
...
class _ScopedPageState extends State<_ScopedPage> {
  ...
  void initState() {
    ...
    _data = _scope.watch(queryKey: ['scoped'], queryFn: (_) async {
      widget.onFetch();
      return 'scoped-data';
    });
  }
