...
class _SimpleGetVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    items = this.useQuery(queryKey: ['items'], queryFn: _fn);
  }
}
...
class _MultiGetVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    a = this.useQuery(queryKey: ['a'], queryFn: _fnA);
    b = this.useQuery(queryKey: ['b'], queryFn: _fnB);
  }
}
...
class _InvalidateGetVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    items = this.useQuery(queryKey: ['inv-items'], queryFn: _fn);
  }
}
...
class _MutateGetVM extends GetBaseViewModel {
  ...
  void onInit() {
    super.onInit();
    items = this.useQuery(queryKey: ['mut-items'], queryFn: _queryFn);
  }
}
...
class _StandaloneVM extends BaseViewModel {
  ...
  void init() {
    super.init();
    items = this.useQuery(queryKey: ['sa-items'], queryFn: _fn);
  }
}
...
void main() {
  ...
  group('QueryResult', () {
    ...
    test('success: isSuccess=true data correct', () async {
      ...
      final r = useQuery(queryKey: ['qr-ok'], queryFn: (_) async => 'hello', client: client);
      await pump();
    ...
    test('error: isError=true error contains exception', () async {
      ...
      final r = useQuery<String>(
        queryKey: ['qr-err'],
        queryFn: (_) async => throw Exception('boom'),
        client: client,
      );
      await pump();
    ...
    test('isFetching=true while request is in flight', () async {
      ...
      final r = useQuery(queryKey: ['qr-fetch'], queryFn: (_) => completer.future, client: client);
      await pump(1);
  ...
  group('useQuery - basic', () {
    test('fetches data successfully', () async {
      final client = freshClient();
      final r = useQuery(queryKey: ['basic-ok'], queryFn: (_) async => 'result', client: client);
      await pump();
      expect(r.isSuccess, true);
    ...
    test('shows error on failure', () async {
      ...
      final r = useQuery<String>(
        queryKey: ['basic-fail'],
        queryFn: (_) => Future.error(Exception('err')),
        client: client,
      );
      await pump();
      expect(r.isError, true);
    ...
    test('placeholder shown before data arrives', () async {
      final client = freshClient();
      final completer = Completer<String>();
      final r = useQuery(queryKey: ['basic-ph'], queryFn: (_) => completer.future,
    ...
    test('seed provides initial cached data', () async {
      final client = freshClient();
      final completer = Completer<String>();
      final r = useQuery(queryKey: ['basic-seed'], queryFn: (_) => completer.future,
    ...
    test('multiple dispose does not throw', () async {
      final client = freshClient();
      final r = useQuery(queryKey: ['multi-dispose'], queryFn: (_) async => 'x', client: client);
      await pump();
      r.dispose();
      expect(() => r.dispose(), returnsNormally);
    });
  ...
  group('useQuery - enabled', () {
    test('enabled:false skips request', () async {
      final client = freshClient();
      var count = 0;
      final r = useQuery(queryKey: ['en-false'], queryFn: (_) async { count++; return 'data'; },
    ...
    test('enabled:true triggers request', () async {
      final client = freshClient();
      var count = 0;
      final r = useQuery(queryKey: ['en-true'], queryFn: (_) async { count++; return 'data'; },
    ...
    test('RxBool enabled=false then true triggers fetch', () async {
      ...
      final r = useQuery(queryKey: ['en-rx'], queryFn: (_) async { count++; return 'rx-data'; },
    ...
    test('RxBool true->false stops refetch after invalidate', () async {
      ...
      final r = useQuery(queryKey: ['en-rx-off'], queryFn: (_) async { count++; return 'v$count'; },
          client: client, enabled: enabled);
      await pump();
      expect(count, 1);
  ...
  group('useQuery - reactive queryKey', () {
    test('Rx item in key triggers refetch on change', () async {
      ...
      final r = useQuery(
        ...
        queryFn: (ctx) async { final key = ctx.queryKey.last as String; fetched.add(key); return 'profile-$key'; },
        client: client,
      );
      await pump();
      expect(fetched, ['u1']);
  ...
  group('useQuery - staleDuration', () {
    test('fresh data: second mount does not refetch', () async {
      final client = freshClient();
      var count = 0;
      final r1 = useQuery(queryKey: ['stale-key'], queryFn: (_) async { count++; return 'data'; },
          client: client, staleDuration: StaleDuration(minutes: 5));
      await pump();
      expect(count, 1);
      r1.dispose();

      final r2 = useQuery(queryKey: ['stale-key'], queryFn: (_) async { count++; return 'data'; },
          client: client, staleDuration: StaleDuration(minutes: 5));
      await pump();
      expect(count, 1);
      expect(r2.data, 'data');
    ...
    test('staleDuration=zero: remount always refetches', () async {
      final client = freshClient();
      var count = 0;
      final r1 = useQuery(queryKey: ['stale-zero'], queryFn: (_) async { count++; return 'v$count'; },
          client: client, staleDuration: StaleDuration.zero);
      await pump();
      expect(count, 1);
      r1.dispose();

      final r2 = useQuery(queryKey: ['stale-zero'], queryFn: (_) async { count++; return 'v$count'; },
          client: client, staleDuration: StaleDuration.zero);
      await pump();
      expect(count, 2);
      r2.dispose();
  ...
  group('useQuery - retry', () {
    ...
    test('error shown after max retries exceeded', () async {
      ...
      final r = useQuery<String>(
        queryKey: ['retry-fail'],
        queryFn: (_) async { attempts++; throw Exception('always fail'); },
        client: client,
  ...
  group('useQuery - deduplication', () {
    test('same key shares single request', () async {
      final client = freshClient();
      var count = 0;
      final r1 = useQuery(queryKey: ['dedup'], queryFn: (_) async { count++; return 'shared'; }, client: client);
      final r2 = useQuery(queryKey: ['dedup'], queryFn: (_) async { count++; return 'shared'; }, client: client);
      await pump();
      expect(count, 1);
    ...
    test('second subscriber still receives data after first disposes', () async {
      final client = freshClient();
      final completer = Completer<String>();
      final r1 = useQuery(queryKey: ['dedup2'], queryFn: (_) => completer.future, client: client);
      final r2 = useQuery(queryKey: ['dedup2'], queryFn: (_) => completer.future, client: client);
      await pump(1);
      r1.dispose();
  ...
  group('useQuery - lifecycle', () {
    test('no updates received after dispose', () async {
      final client = freshClient();
      final completer = Completer<String>();
      final r = useQuery(queryKey: ['lc-disp'], queryFn: (_) => completer.future, client: client);
      await pump(1);
      expect(r.data, null);
      r.dispose();
    ...
    test('invalidateQueries triggers refetch', () async {
      final client = freshClient();
      var count = 0;
      final r = useQuery(queryKey: ['lc-inv'], queryFn: (_) async { count++; return 'v$count'; }, client: client);
      await pump();
      expect(count, 1);
  ...
  group('useQueryClient', () {
    ...
    test('returns the registered instance\'s client, custom or default alike', () async {
      final svc = Get.put<QueryService>(
        QueryService(staleDuration: StaleDuration(minutes: 1)),
        permanent: true,
      );
      // useQueryClient() has no separate "default singleton" to fall back to —
      // whichever QueryService instance is registered IS the global one.
      expect(useQueryClient(), same(svc.client));
      expect(svc.client.defaultQueryOptions.staleDuration, StaleDuration(minutes: 1));
    });
  });

  group('QueryService - custom defaults', () {
    test('per-field params override just that field, keep other getx_query defaults', () async {
      final svc = QueryService(gcDuration: GcDuration(minutes: 30));
      Get.put<QueryService>(svc, permanent: true);
      await pump();
      expect(svc.client.defaultQueryOptions.gcDuration, GcDuration(minutes: 30));
      // Untouched fields keep getx_query's own defaults, not flutter_query's raw ones.
      expect(svc.client.defaultQueryOptions.staleDuration, StaleDuration(minutes: 5));
      expect(svc.client.defaultQueryOptions.retry, isNotNull);
    });

    test('multiple fields can be overridden independently', () async {
      final svc = QueryService(
        staleDuration: StaleDuration(minutes: 1),
        gcDuration: GcDuration(minutes: 30),
      );
      Get.put<QueryService>(svc, permanent: true);
      await pump();
      expect(svc.client.defaultQueryOptions.staleDuration, StaleDuration(minutes: 1));
      expect(svc.client.defaultQueryOptions.gcDuration, GcDuration(minutes: 30));
      // retry wasn't overridden — still getx_query's default.
      expect(svc.client.defaultQueryOptions.retry, isNotNull);
    });

    test('no constructor args keeps getx_query\'s own hardcoded defaults', () async {
      final svc = QueryService();
      Get.put<QueryService>(svc, permanent: true);
      await pump();
      expect(svc.client.defaultQueryOptions.staleDuration, StaleDuration(minutes: 5));
      expect(svc.client.defaultQueryOptions.gcDuration, GcDuration(minutes: 10));
      expect(svc.client.defaultQueryOptions.retry, isNotNull);
    });
  });
  ...
  group('GetBaseViewModel', () {
    ...
    test('prefetchQuery warms cache', () async {
      ...
      vm.prefetchQuery(queryKey: ['prefetch-key'], queryFn: (_) async { count++; return 'pf'; });
      await pump();
  ...
  group('QueryScope', () {
    test('watch collects results, dispose clears all', () async {
      ...
      final a = scope.watch(queryKey: ['sc-a'], queryFn: (_) async { countA++; return 'a$countA'; });
      final b = scope.watch(queryKey: ['sc-b'], queryFn: (_) async { countB++; return 'b$countB'; });
      await pump();
    ...
    test('QueryScope.invalidateQueries works', () async {
      ...
      var count = 0;
      final scope = QueryScope(client: client);
      final r = scope.watch(queryKey: ['sc-inv'], queryFn: (_) async { count++; return 'v$count'; });
      await pump();
      expect(count, 1);
    ...
    test('QueryScope.mutate helper invalidates on success', () async {
      ...
      var count = 0;
      final scope = QueryScope(client: client);
      final r = scope.watch(queryKey: ['sc-mut'], queryFn: (_) async { count++; return 'v$count'; });
      await pump();
      expect(count, 1);
    ...
    test('QueryScope uses useQueryClient when no client passed', () async {
      ...
      var called = false;
      final r = scope.watch(queryKey: ['sc-global'], queryFn: (_) async { called = true; return 'global'; });
      await pump();
      expect(called, true);
      expect(r.isSuccess, true);
      scope.dispose();
  ...
  // ── 15. reactive collection types (RxList/RxMap/RxSet) ──────────────────

  group('useQuery - reactive queryKey (RxList)', () {
    test('RxList item in key triggers refetch when mutated', () async {
      final client = freshClient();
      final tags = <String>['a'].obs;
      var count = 0;
      final r = useQuery(
        queryKey: ['tagged', tags],
        queryFn: (_) async { count++; return 'v$count'; },
        client: client,
      );
      await pump();
      expect(count, 1);
  ...
  group('QueryResult - dispose guard', () {
    test('refetch() after dispose is a no-op, does not throw', () async {
      final client = freshClient();
      final r = useQuery(queryKey: ['dg-refetch'], queryFn: (_) async => 'x', client: client);
      await pump();
      r.dispose();
      await expectLater(r.refetch(), completes);
    });
  });

  group('MutationResult - dispose guard', () {
    test('mutate() after dispose is a no-op, does not throw', () async {
      final client = freshClient();
      final m = useMutation<String, String>((v, _) async => v, client: client);
      m.dispose();
      expect(() => m.mutate('x'), returnsNormally);
    });

    test('mutateAsync() after dispose throws StateError', () async {
      final client = freshClient();
      final m = useMutation<String, String>((v, _) async => v, client: client);
      m.dispose();
      await expectLater(m.mutateAsync('x'), throwsA(isA<StateError>()));
    });

    test('reset() after dispose is a no-op, does not throw', () async {
      final client = freshClient();
      final m = useMutation<String, String>((v, _) async => v, client: client);
      m.dispose();
  ...
  // ── 17. useIsFetching / useIsMutating live reactivity ────────────────────

  group('useIsFetching / useIsMutating', () {
    test('useIsFetching updates reactively while a query is in flight', () async {
      final client = freshClient();
      final completer = Completer<String>();
      final (count, disposeCount) = useIsFetching(client: client);
      expect(count.value, 0);

      final r = useQuery(
        queryKey: ['fetching-live'],
    ...
    test('useIsMutating updates reactively while a mutation is pending', () async {
      final client = freshClient();
      final completer = Completer<String>();
      final (count, disposeCount) = useIsMutating(client: client);
      expect(count.value, 0);

      final m = useMutation<String, String>((v, _) => completer.future, client: client);
      m.mutate('x');
      await pump(1);
      expect(count.value, greaterThan(0));
  ...
  group('useInfiniteQuery', () {
    Future<List<int>> fetchPage(int page) async => page < 3 ? [page] : [];

    test('fetches first page on mount', () async {
      final client = freshClient();
      final feed = useInfiniteQuery<List<int>, int>(
        queryKey: ['feed-basic'],
    ...
    test('fetchNextPage appends a page', () async {
      final client = freshClient();
      final feed = useInfiniteQuery<List<int>, int>(
        queryKey: ['feed-next'],
        queryFn: (ctx) => fetchPage(ctx.pageParam),
    ...
    test('hasNextPage becomes false once the query is exhausted', () async {
      final client = freshClient();
      final feed = useInfiniteQuery<List<int>, int>(
        queryKey: ['feed-exhaust'],
        queryFn: (ctx) => fetchPage(ctx.pageParam),
        initialPageParam: 0,
        nextPageParamBuilder: (data) =>
            data.pages.last.isEmpty ? null : data.pages.length,
      ...
      await pump();
      await feed.fetchNextPage(); // page 1
      await pump();
      await feed.fetchNextPage(); // page 2
      await pump();
      expect(feed.pages, [
        [0],
        [1],
      ...
      await feed.fetchNextPage(); // page 3 -> empty, no more pages
      await pump();
      expect(feed.hasNextPage, false);
      feed.dispose();
    });

    test('actions after dispose are no-ops', () async {
      final client = freshClient();
      final feed = useInfiniteQuery<List<int>, int>(
        queryKey: ['feed-disposed'],
        queryFn: (ctx) => fetchPage(ctx.pageParam),
        initialPageParam: 0,
        nextPageParamBuilder: (data) =>
            data.pages.last.isEmpty ? null : data.pages.length,
      ...
      await pump();
      feed.dispose();
      await expectLater(feed.fetchNextPage(), completes);
      await expectLater(feed.refetch(), completes);
    });

    test('GetBaseViewModel useInfiniteQuery auto-disposes on onClose', () async {
      final client = freshClient();
      final vm = Get.put(_FeedVM(client));
      await pump();
      expect(vm.feed.isSuccess, true);
      expect(vm.feed.pages, [
        [0]
      ]);
      expect(() => Get.delete<_FeedVM>(), returnsNormally);
    });
  });

  // ── 19. useQueries combine ────────────────────────────────────────────────

  group('useQueries - combine', () {
    test('combined() derives a value from all results', () async {
      final client = freshClient();
      final (results, combined, disposeAll) = useQueries(
        [
          QueryOptions(['cmb-a'], (_) async => 'a'),
          QueryOptions(['cmb-b'], (_) async => 'b'),
        ],
        client: client,
        combine: (rs) => rs.every((r) => r.isSuccess),
      );
      expect(combined(), false);
      await pump();
    ...
    test('combined() throws if no combine callback was provided', () async {
      final client = freshClient();
      final (results, combined, disposeAll) = useQueries(
        [QueryOptions(['cmb-none'], (_) async => 'x')],
        client: client,
      );
      await pump();
      expect(results.first.isSuccess, true);
      expect(() => combined(), throwsStateError);
      disposeAll();
    });
  });
}
...
class _FeedVM extends GetBaseViewModel {
  _FeedVM(QueryClient client) : super(client: client);
  late InfiniteQueryResult<List<int>, int> feed;

  @override
  void onInit() {
    super.onInit();
    feed = this.useInfiniteQuery<List<int>, int>(
      queryKey: ['feed-vm'],
      queryFn: (ctx) async => ctx.pageParam < 3 ? [ctx.pageParam] : <int>[],
      initialPageParam: 0,
      nextPageParamBuilder: (data) =>
          data.pages.last.isEmpty ? null : data.pages.length,
    );
