...
**roadmap** 是百家乐/龙虎斗/骰宝路子图（露珠图）的 Flutter 实现，移植自 casino monorepo 的 `apps/baccarat-roadmap`（TypeScript，`main` 分支）。核心设计沿用 TS 版本的三层架构：`core`（纯 Dart 算法层，零 Flutter 依赖）→ `render`（消费算法层产出的绘制指令）→ `panel`（Flutter widget 外壳：手势、视口、动画、回放、UX 增强）。
...
roadmap 不是逐行翻译，`core/` 之外的两层按 Flutter 的原生能力做了针对性简化：
...
```
lib/
  roadmap.dart              # 库入口（barrel export）
  src/
