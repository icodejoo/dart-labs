...
- mask 未读取 SVG 的 `maskUnits`/`x`/`y`/`width`/`height`(即不按官方默认的 bbox ±10%/120% 去**裁剪** mask 区域;mask 内容画到哪里就覆盖到哪里)。注意这与"离屏图层分配多大"是两件事:后者自 2026-08-27 起按 mask 内容自身几何边界分配(`SvgXAnimationQuality.tightMaskLayerBounds`,默认开启、**无损**,真实语料 488 次全帧比对逐位一致),而不再按整个画布分配。
...
- **`tightMaskLayerBounds`(无损,默认开启,不受并发阈值门控)**:`<mask>` 的两个 `saveLayer` 按 mask 内容自身边界分配,而不是留成无界(=按整个 SVG 视口分配),真实语料离屏面积平均少 21.5%,代价 +4.1~5.5µs/图标/帧(主机侧)。像素等价而非近似:mask 边界之外的内容本来就会被 `dstIn` 擦掉。`<text>` mask 与涉及模糊的情形自动回退无界。另附一项精确收益:mask 覆盖度整体在可见裁剪区之外时(还没滑进来的揭示 mask)整个被遮罩子树直接跳过,两个离屏通道都不开。
