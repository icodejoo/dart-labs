...
// pure Dart numeric work (see svgx CLAUDE.md: per-frame work must not cross FFI).
//
// If the native library isn't loaded (plain `flutter test` on the host VM, where
// no Flutter build step produced `svgx.dll`/`.so`), every helper degrades to
...
// 一次，结果存进文档模型，因此逐帧采样/绘制路径仍是纯 Dart 数值运算（见 svgx
// CLAUDE.md：每帧的活不得跨 FFI）。
//
// 若原生库未加载（纯 `flutter test` 的 host VM 下没有 Flutter 构建步骤产出的
// `svgx.dll`/`.so`），所有函数退化为返回 `null` 而非抛错——符合本引擎"不支持的
