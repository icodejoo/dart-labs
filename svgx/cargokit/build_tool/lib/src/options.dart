...
/// SVGX-LOCAL MODIFICATION of upstream cargokit — see [CargoBuildOptions].
/// Upstream restricts `toolchain:` to the [Toolchain] enum (stable/beta/
/// nightly only), which can't express a pinned dated nightly like
/// `nightly-2026-06-24` (needed for reproducible `-Z build-std` release
/// builds — see `rust/cargokit.yaml`). This project needs that, so
/// `toolchain` is now a plain String accepted as-is and passed straight to
/// `rustup`/`cargo +<toolchain>`, instead of being restricted to the three
/// built-in enum values.
///
/// SVGX-LOCAL 对上游 cargokit 的修改——见 [CargoBuildOptions]。上游把
/// `toolchain:` 限制为 [Toolchain] 枚举（仅 stable/beta/nightly），无法表达
/// `nightly-2026-06-24` 这种带日期的固定 nightly（本项目 `-Z build-std`
/// release 构建的可复现性需要它，见 `rust/cargokit.yaml`）。本项目需要这个
/// 能力，因此 `toolchain` 改为按原样接受的普通字符串，直接传给
/// `rustup`/`cargo +<toolchain>`，不再限制为三个内置枚举值。
class CargoBuildOptions {
  final String toolchain;
  final List<String> flags;

  /// SVGX-LOCAL: extra environment variables (string -> string), parsed from
  /// the `env:` map in `cargokit.yaml`. Absent upstream, which has no way to
  /// pass extra env (e.g. `RUSTFLAGS`) into the cargo invocation for a given
  /// build configuration.
  ///
  /// SVGX-LOCAL：额外的环境变量（字符串 -> 字符串），从 `cargokit.yaml` 里的
  /// `env:` 映射解析而来。上游没有这个能力，无法为某个构建配置向 cargo
  /// 调用传入额外环境变量（例如 `RUSTFLAGS`）。
  ...
  /// SVGX-LOCAL: accept any rustup toolchain name, not just the three
  /// built-in [Toolchain] enum values — see the class doc comment above.
  ///
  /// SVGX-LOCAL：接受任意 rustup 工具链名，不再局限于内置 [Toolchain] 枚举的
  /// 三个值——见上面的类文档注释。
  static String _toolchainFromNode(YamlNode node) {
    ...
    throw SourceSpanException(
        'Unknown toolchain. Must be a non-empty rustup toolchain name.',
        node.span);
  }

  /// SVGX-LOCAL: parses the `env:` map (string -> string).
  ///
  /// SVGX-LOCAL：解析 `env:` 映射（字符串 -> 字符串）。
  static Map<String, String> _envFromNode(YamlNode node) {
    if (node case YamlMap(nodes: Map<dynamic, YamlNode> map)) {
      final result = <String, String>{};
      for (final entry in map.entries) {
        final key = entry.key;
        final value = entry.value;
        if (key case YamlScalar(value: String keyString)) {
          if (value case YamlScalar(value: String valueString)) {
            result[keyString] = valueString;
            continue;
          }
        }
        throw SourceSpanException(
            'env entries must be string -> string.', value.span);
      }
      return result;
    }
    throw SourceSpanException('env must be a map', node.span);
  }
  ...
  static CargoBuildOptions parse(YamlNode node) {
    ...
    String toolchain = Toolchain.stable.name;
    List<String> flags = [];
    ...
    for (final MapEntry(:key, :value) in node.nodes.entries) {
      if (key case YamlScalar(value: 'toolchain')) {
        ...
        // SVGX-LOCAL: new option, absent upstream.
      } else if (key case YamlScalar(value: 'env')) {
        env = _envFromNode(value);
      } else {
        throw SourceSpanException(
            'Unknown cargo option type. Must be "toolchain", "extra_flags" or "env".',
            key.span);
      }
    }
    return CargoBuildOptions(toolchain: toolchain, flags: flags, env: env);
  }
}
