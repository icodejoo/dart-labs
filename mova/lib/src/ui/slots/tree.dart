...
/// without mutating anything, and by [MovaPatchAdd] to tag an appended
...
/// 时使用（不修改任何原始对象），也供 [MovaPatchAdd] 用来给新增的组件打上
...
class _CopyComponent extends MovaComp {
  ...
  @override
  MovaSlot get slot => slotOverride ?? _source.slot;

  @override
  int get order => orderOverride ?? _source.order;

  @override
  Widget build(BuildContext context, MovaApi api, List<Widget> builtChildren) =>
      _source.build(context, api, builtChildren);
}

/// Groups already-built top-level widgets by [MovaSlot] for a single frame.
///
/// Returned by [buildSlots]; index with a [MovaSlot] to get the ordered
/// widget list for that region, or an empty list if nothing occupies it.
///
/// 把已构建好的顶层 widget 按 [MovaSlot] 分组，供单帧渲染使用。
///
/// 由 [buildSlots] 返回；用 [MovaSlot] 取索引即可得到该区域已排序的 widget
/// 列表，若该区域无内容则返回空列表。
class MovaSlotBundle {
  /// Creates a bundle backed by a pre-grouped [_bySlot] map.
  ///
  /// 用预先分好组的 [_bySlot] map 创建一个 bundle。
  MovaSlotBundle(this._bySlot);

  /// Widgets grouped by slot, already sorted within each slot.
  ///
  /// 按槽位分组的 widget，每个槽位内部已排好序。
  final Map<MovaSlot, List<Widget>> _bySlot;

  /// Returns the ordered widgets for [slot], or an empty list if none.
  ///
  /// 返回 [slot] 对应的已排序 widget 列表，若没有则返回空列表。
  List<Widget> operator [](MovaSlot slot) => _bySlot[slot] ?? const [];
}
...
MovaSlotBundle buildSlots(BuildContext context, MovaApi api, List<MovaComp> tree) {
  ...
  final bySlot = <MovaSlot, List<Widget>>{};
