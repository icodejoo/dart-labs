...
/// effects (not mere rebuilds — those stay stateless via `MovaSelect`).
///
...
/// so host [MovaHook]s stay in the loop.
///
/// 为「事件副作用型」有状态组件提供能力的 mixin（副作用，非单纯重建——纯重建
/// 用 `MovaSelect` 保持无状态即可）。
///
/// 只给两样与业务无关的能力：
///
...
mixin MovaPlugin<T extends StatefulWidget> on State<T> {
  ...
  MovaApi get api => MovaScope.readOf(context);
