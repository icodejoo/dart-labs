...
class MovaSocialRailComponent extends MovaComponent {
  ...
  List<MovaComponent> get children => [
        ...
        MovaShareButtonComponent(item: item),
      ];
...
/// Author avatar; tapping the avatar fires [MovaFeedItem.onAvatarTap], tapping
/// the small follow badge beneath it fires [MovaFeedItem.onFollowTap].
/// mova holds no follow state of its own (see [MovaFeedItem]'s doc
...
class MovaAvatarComponent extends MovaComponent {
  ...
  ///
  /// 创建头像叶子组件。
  MovaAvatarComponent({required this.item});
...
class MovaLikeButtonComponent extends MovaComponent {
  ...
  ///
  /// 创建点赞按钮叶子组件。
  MovaLikeButtonComponent({required this.controller, required this.index, required this.likeNotifier});
...
/// Comment entry; display-only count from [MovaFeedItem.commentCount], tap
/// fires [MovaFeedItem.onComment] — mova owns no comment UI.
///
...
class MovaCommentButtonComponent extends MovaComponent {
  ...
  ///
  /// 创建评论按钮叶子组件。
  MovaCommentButtonComponent({required this.item});
...
/// Share entry; display-only count from [MovaFeedItem.shareCount], tap fires
/// [MovaFeedItem.onShare] — mova owns no share sheet.
///
...
class MovaShareButtonComponent extends MovaComponent {
  ...
  ///
  /// 创建分享按钮叶子组件。
  MovaShareButtonComponent({required this.item});
...
class MovaFeedInfoComponent extends MovaComponent {
  ...
  ///
  /// 创建 feed 信息叶子组件。
  MovaFeedInfoComponent({required this.item});
