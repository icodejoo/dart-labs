...
///
/// Presentation-only and reusable: it never reads MovaApi itself, only the
/// [MovaTheme] it is given — the calling component is responsible for reading
/// `api.options.theme` and passing it down.
///
/// 控制条中通用的带主题小图标按钮。
///
/// 仅负责展示、可复用：它自身从不读取 MovaApi，只使用传入的 [MovaTheme]——由
/// 调用方组件负责读取 `api.options.theme` 并向下传递。
class MovaIconButton extends StatelessWidget {
  /// The glyph to show.
  ///
  /// 要显示的图标。
  final IconData icon;
  ...
  Widget build(BuildContext context) {
    ...
    // MovaScope/component tree, not a Scaffold) does not always provide.
    //
    // 用普通 GestureDetector 而非 InkResponse/InkWell：后者需要 Material
    // 祖先节点，而播放器叠加层（经 MovaScope/组件树组合，并非 Scaffold）不一定
    // 提供这个祖先。
...
///
/// Presentation-only and reusable: it never reads MovaApi itself, only the
/// [MovaTheme] it is given.
///
/// 半透明的顶/底渐变条容器；空白区域不吞点击事件（让中间手势穿透）。
///
/// 仅负责展示、可复用：它自身从不读取 MovaApi，只使用传入的 [MovaTheme]。
class MovaGradBar extends StatelessWidget {
