...
abstract class MovaSkin {
  ...
  /// `MovaPlayer`, not by this skin). [slots] holds every top-level component
  /// already built and grouped by [MovaSlot] via [buildSlots]. Returns the
  /// composed [Widget] to mount.
  ///
  /// 把已构建好的 [slots] 与原始视频画面 [video] 合成为 [context] 下的最终
  /// widget 树。
  ///
  /// [video] 是已渲染好的视频画面（由调用方，例如 `MovaPlayer`，持有，本皮肤
  /// 不拥有它）。[slots] 是通过 [buildSlots] 已构建并按 [MovaSlot] 分组的全部
