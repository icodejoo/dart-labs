...
/// Runs a list of [MovaHook]s in order, short-circuiting on veto/cancel.
///
/// 按顺序运行一组 [MovaHook]，遇到否决/取消即短路返回。
class MovaHookChain {
  /// The ordered interceptors to consult.
  ///
  /// 依次咨询的拦截器列表。
  final List<MovaHook> interceptors;

  /// Creates a chain over [interceptors], consulted in list order.
  ///
  /// 基于 [interceptors] 创建拦截链，按列表顺序依次咨询。
  const MovaHookChain(this.interceptors);
