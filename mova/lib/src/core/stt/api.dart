...
/// The speech-to-text subtitle capability surface exposed on `MovaApi.stt`.
///
/// UI components only ever listen to [cues]/[current] and call
/// [start]/[stop]; the audio pipeline (feeding decoded PCM into whichever
/// [MovaSttEngine] is configured) lives behind this abstraction in the core
/// layer.
///
/// 挂在 `MovaApi.stt` 上的语音转字幕能力面。
///
/// UI 组件只会监听 [cues]/[current] 并调用 [start]/[stop]；音频管线（把解码后
/// 的 PCM 喂给配置好的 [MovaSttEngine]）都藏在该抽象之后的 core 层里。
abstract class MovaSttApi {
  /// Language codes the active engine recognizes; empty when no engine is
  /// configured (see `MovaSttConfig.engine`).
  ///
  /// 当前引擎能识别的语言代码；未配置引擎时为空（见 `MovaSttConfig.engine`）。
