...
/// re-watched.
///
/// A distinct concept from [MovaSttEngine]'s live `feed`/`cues` loop: this
...
/// 不管重播多少次都只转写一次。
///
/// 与 [MovaSttEngine] 那套 `feed`/`cues` 实时循环是不同的概念：本存储持有的是
/// 一次性批量转写的**完成结果**（见
...
abstract class MovaSttSubStore {
  ...
  ///   媒体源的稳定标识，通常是其 URI
  Future<List<MovaSttCue>?> load(String sourceKey);
  ...
  Future<void> save(String sourceKey, List<MovaSttCue> cues);
...
class MovaFileSttSubStore implements MovaSttSubStore {
  ...
  @override
  Future<void> save(String sourceKey, List<MovaSttCue> cues) async {
    final file = await _fileFor(sourceKey);
