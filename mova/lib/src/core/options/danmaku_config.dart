...
/// 业务。
class MovaDanmakuConfig {
  /// Master switch; the track renders nothing at all when `false`.
  ///
  /// 总开关；为 `false` 时轨道完全不渲染。
  final bool enabled;

  /// The fixed list of comments to render, keyed by [MovaDanmakuItem.time].
  ///
  /// 要渲染的固定弹幕列表，按 [MovaDanmakuItem.time] 触发。
  final List<MovaDanmakuItem> items;
  ...
  const MovaDanmakuConfig({
    this.enabled = false,
    this.items = const <MovaDanmakuItem>[],
    this.trackCount = 4,
  ...
  MovaDanmakuConfig copyWith({
    bool? enabled,
    List<MovaDanmakuItem>? items,
    int? trackCount,
    double? trackHeight,
    double? fontSize,
    double? opacity,
