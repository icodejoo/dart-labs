...
/// Creates one fully-wired playback engine for [MovaFeedEnginePool] to own.
///
/// Hosts pass `createMovaEngine` here; tests pass a fake factory. The pool
/// calls this at most [MovaFeedEnginePool.size] times over its whole lifetime
/// and disposes every engine it created.
///
/// 为 [MovaFeedEnginePool] 创建一个已完整装配的播放引擎。
///
/// 宿主传 `createMovaEngine`，测试传假工厂。整个生命周期内池最多调用它
/// [MovaFeedEnginePool.size] 次，并负责释放自己创建的每个引擎。
///
...
///
/// [ready] is `false` between [MovaFeedEnginePool.bind] being called and its
...
///
/// 从调用 [MovaFeedEnginePool.bind] 到其 `open()` resolve 之间，[ready] 为
...
class MovaFeedSlot {
  ...
  const MovaFeedSlot({required this.api, required this.ready});
...
/// One engine's current binding inside [MovaFeedEnginePool]; mutable internal
/// bookkeeping behind the immutable [MovaFeedSlot] snapshots handed out.
///
/// [MovaFeedEnginePool] 内部某个引擎的当前绑定；是对外发放的不可变
/// [MovaFeedSlot] 快照背后的可变簿记。
class _Binding {
  ...
  /// [MovaSource] has no value equality.
  ///
  /// 该绑定打开的源 uri；按字符串比较，因为 [MovaSource] 没有值相等。
  final String uri;
...
class MovaFeedEnginePool {
  ...
  ///   对其应用一次的填充模式
  /// - [onChanged]: fired whenever any slot's binding or [MovaFeedSlot.ready]
  ...
  /// 引擎"列表。
  final List<MovaApi> _idle = <MovaApi>[];
  ...
  MovaFeedSlot? slotFor(int index) {
    final binding = _bound[index];
    if (binding == null) return null;
    return MovaFeedSlot(api: binding.api, ready: binding.ready);
  }
  ...
  Future<void> dispose() async {
    if (_disposed) return;
    _disposed = true;
    final engines = <MovaApi>[..._idle, ..._bound.values.map((b) => b.api)];
    _bound.clear();
    _idle.clear();
...
List<int> movaFeedWindow(int center, int size) {
