...
///
/// Kept separate from [MovaState] because it changes at a much higher
...
/// 播放位置与缓冲进度的不可变快照。
///
/// 与 `MovaState` 分离，因为它的变化频率远高于状态（播放期间每 tick 都变），
...
class MovaProg {
  ...
  ///
  /// 创建一个进度快照；默认零/零。
  const MovaProg({this.position = Duration.zero, this.buffer = Duration.zero});

  /// Returns a copy with the given fields replaced.
  ///
  /// 返回一个替换了指定字段的副本。
  MovaProg copyWith({Duration? position, Duration? buffer}) {
    return MovaProg(
  ...
  bool operator ==(Object other) {
    ...
    return other is MovaProg && other.position == position && other.buffer == buffer;
  }
