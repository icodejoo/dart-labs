...
/// memory does keeping a *second* [MovaEngine] warm (opened and parked on its
...
/// 保持*第二个* [MovaEngine] 热着（已 open 并停在首帧、随时可切入），真实内存
/// 代价到底是多少？
///
...
class _DualEngineMemorySpikePageState extends State<DualEngineMemorySpikePage> {
  ...
  /// 一致。
  late final MovaEngine _primary;
  ...
  void initState() {
    super.initState();
    _primary = createMovaEngine();
    _primary.open(const MovaSource(_spikeSourceA));
  }
  ...
  Future<void> _addPreload() async {
    ...
    await engine.open(const MovaSource(_spikeSourceB), autoPlay: false);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            Expanded(child: MovaPlayer(api: _primary, autoLoadQualities: false)),
            Padding(
