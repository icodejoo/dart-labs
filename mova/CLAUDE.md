...
未发生原生崩溃；F 组既有 demo 无回归。
**同日 Android 真机（STG AL00）补测 C 组三项专属项，发现并修复一个真实 bug**：
① **转屏钳回——发现真实 bug，已修复**：连续横竖屏切换后小窗从贴住的角落
逐渐"走位"，根因是原逻辑对 bounds 变化只做 `clampToBounds`（合法性钳制，不
保证相对位置），转屏导致 bounds 宽高互换时旧的绝对 left/top 不再对应原视觉
位置。已加 `core/mini/placement.dart` 的 `remapProportionally` 纯函数（按
旧 bounds 里的相对比例重新映射到新 bounds，再钳制兜底），`mini_window.dart`
接入，新增 3 项单测，复测确认转屏后稳定贴住原角落附近；② **与系统 PiP 互斥
——真机确认通过**：小窗态下触发 `enterPip()`，先 `setMini(false)` 收起小窗
再进系统 PiP，未叠加/花屏（demo 首页新增了触发按钮）；③ **切后台再回前台
——播放会自动暂停，需手动点续播**：排查确认这**不是** mini window 引入的
bug（代码里完全没有 `AppLifecycleState`/`WidgetsBindingObserver` 相关处理），
是 Android 系统级行为（切后台时 GPU 渲染 Surface 被回收，libmpv 检测到
surface 丢失自动暂停），全屏播放切后台同样会暂停，非小窗特有——若要"前后台
自动续播"需要单独立项评估（foreground service 或生命周期监听 + 手动
`resume()`），不阻塞本次验收结论。
**验证过程中还发现并修复一个 demo 自身次生 bug**：`_MisuseDemoState` 触发
   ...
   **2026-09-24（同一台 STG AL00）追加验证，新建 `example/lib/main_seamless_swap_verify.dart`
   ...
   - **中插续播点误差——首次测量方向搞反了，已定位真实根因并修复**：初版探针
     ...
     **真机复测**（新探针 `example/lib/main_resume_accuracy_verify.dart`，未
     ...
     一个采样格，已接近测量下限。**顺带发现的第二个疑点——已排查，确认不是
     代码 bug**：`_startWarm` 里 `open()` 后立刻下发的 `seek(at)` 曾观察到
     3 次运行 1 次快（20ms）、2 次慢（2.4s 才落地）。真机对照实验（裸
     media_kit 三变体）证实：`open()` 后立刻 seek 若无寄存机制会被 mpv 丢弃
     且**卡死播放器**（position 5 秒内不离开 0、`dispose()` 也挂住）；mova
     的寄存机制 12/12 轮全部正确走到、全部真实落地，0 轮丢弃。那 2.4 秒就是
     mpv 报出 duration 本身的网络加载耗时（实测 3068–5327ms 不等），与寄存
     机制无关——广告只播 4 秒、影子光加载到能 seek 就要 3-5 秒，ready 有时
     赶在 skip 前、有时赶不上，这就是分布本身。**顺手做了一次加固**（不是
     修复这个延迟）：`MovaEngine.open()` 里同步重置 `state.duration`/
     `_lastPosition`/`_lastBuffer` 为 0，把"寄存还是直发"的判据从一个隐性
     竞态（此前依赖 media_kit 内部"duration 归零"恰好先于 seek 到达，实测
     每轮都赢但终究是竞态，输的一侧是卡死）变成确定性的；顺带修掉一个真
   ...
   - **内存/解码 session 三阶段采样——方法论问题已解决（独立进程），但发现
     一个稳定复现、未定性的现象**：新建
     `example/lib/main_swap_memory_verify.dart`（未提交），不再与其他测试组
     共享进程，两轮独立进程重启，baseline 两轮相近（132.90/135.59 MiB），
     无负增量，进程隔离方法论有效。**第一轮**：baseline=135.59MiB → 双引擎
     并存=148.36MiB(+12.78) → 切回正片=166.57MiB(+30.98)。**第二轮**：
     baseline=132.90MiB → 双引擎并存=154.14MiB(+21.25) → 切回正片=
     166.46MiB(+31.56)。**两轮一致地未回落到接近 baseline，反而比双引擎并存
     阶段更高**（+31MiB 左右，不是偶发）；阶段②（双引擎并存）本身两轮波动
     较大（+12.78 vs +21.25，样本量只有 2 次，无法判断是否属正常范围）。
     **已定性排查，结论：不是内存泄漏**。新建
     `example/lib/main_swap_leak_probe.dart`（未提交），把三阶段改成重复
     8 轮"插播广告→skip→原子切回正片"，同时用 `adb shell dumpsys meminfo`
     每 10s 采一次分栏。真机数字（MODE=swap）：baseline=128.98MiB →
     round1=159.47(+30.49) → round2=180.37(+51.39) → round3~8 稳定在
     164.20~169.12MiB（**逐轮增量 −0.25~+0.77MiB，噪声量级，不是线性累加**）
     → 静置 60s 持平（169.55~170.06）→ dispose 后回落到 153.05MiB。
     `dumpsys meminfo` 显示 **Native Heap** 首轮双引擎峰值 53.0MB，第 2 轮起
     稳定在 39.5–41.0MB（4 分钟漂移 ≈1.5MB）；**EGL mtrack 全程恒定
     51040/56104 KB，8 轮=16 次引擎创建/销毁一次都没长过**——纹理泄漏是
     最可疑的图形资源泄漏路径，这里零增长直接排除。**真泄漏的特征是逐轮
     线性累加，实测是"第 1-2 轮一次性抬升约 35MiB 后进入平台"**：baseline
     是进程只创建过一个引擎时的水位，第一次双引擎并存把 native 分配器
     （scudo/jemalloc arena）和图形分配的高水位一次性推高，之后释放的内存
     被后续轮次复用而不归还 OS，是一次性水位抬升，不是累积泄漏。代码层面也
     确认了释放链完整：`MovaSwapEngine._commitNow()` 所有分支
     （`abandon`/`_onShadowError`/`dispose`）都对旧引擎调用了
     `dispose()`，media_kit_video 的 `AndroidVideoController` 把纹理释放
     挂在 `Player.dispose()` 上，没有漏引用的分支。硬切对照组（用于归因
     "抬升来自第二引擎还是 open churn"）因 USB 重新枚举中断未跑完，但对
     "是否泄漏"这个判定不构成影响，平台化+EGL 恒定已单独成立。
   - **广告黑屏是否真的消除、切换瞬间画面是否跳变——已实测确认，音频跳变仍受限
     于设备无可用采集路径**（2026-09-25，同一台 STG AL00）：`adb shell
     screenrecord` 在该机型上不可用（系统精简掉了），改用 `adb shell
     screencap` 在设备端连续抓帧、与 logcat 的 `renderEpoch`/skip 事件时间戳
     对齐，约 250–330ms 一帧密集覆盖 skip→resume 整个窗口（新建极简视觉探针
     `example/lib/main_swap_visual_probe.dart`，未提交，仅播放器占满全屏+底部
     状态行）。**未观察到黑屏帧**——采样内每一帧要么是完整广告画面、要么是
     完整正片画面，没有过渡黑帧/色帧；受限于单帧 `screencap` 耗时约
     270–700ms，不能做逐 ms 级穷尽证明（不能 100% 排除小于采样间隔的黑屏），
     但已是当前工具条件下能拿到的最强客观证据，且该量级本身已接近人眼无感
     范围。**画面切换处未见撕裂/跳变**——正片切回后位置自然连续推进（实测
     一轮：skip 于 12.383s，帧 40 在 12.962s 已显示"正片"标题+进度 00:04，
     非重播/非跳到无关位置）。**音频跳变仍如实测不了**：该设备无可用音频
     采集路径（`screencap` 不采音频，`screenrecord` 本机不可用），这部分保留
     为局限，不编造结论。
