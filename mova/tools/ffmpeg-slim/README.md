...
| iOS（ios-arm64 only） | 🟡 CI 构建通过，体积优化已到本轮上限，**未接入 mova 工程、未做任何真机播放验证** | 走 `media-kit/libmpv-darwin-build`（iOS/macOS 共用一个仓库），`movaslim` flavor（ffmpeg n9.0 + mpv v0.41.0 + libplacebo/OpenGL，硬解 VideoToolbox）。2026-08-11 四轮 CI 验证过的瘦身：① size flags（`-fvisibility=hidden`/`-ffunction-sections -fdata-sections`/`-Wl,-dead_strip`）12,374,408 B → 10,967,592 B（−11.4%，CI run 31454544613）；② 全链路 LTO（ffmpeg `--enable-lto` + cross-file `-flto`）→ 10,666,384 B（累计 −13.8%，CI run 31456135129）；③④ 关 glslang/shaderc 和 libxml2（两个都确认是真死代码——movaslim 从不触发 DASH/Vulkan/D3D11，但 LTO+dead_strip 早就吃干净了，两步各自只再省 16KB/0B，CI run 31457206131、31458125814）。**最终定格 10,650,000 B（≈10.15MiB，累计 −13.9%）**。

　　之前一直拿 Android 定稿的 6.52MiB 做对比不公平——那是 **ffmpeg 6.0 + mpv 0.36（v6 线，无 libplacebo）**，iOS 走的是 **ffmpeg n9.0 + mpv 0.41（v9 线，libplacebo 强制依赖）**，同口径应该比 Android 自己的 v9 实验结果（9.30MB，见下方"Darwin movaslim flavor"一节），真实差距只有 ~0.85MB。查证 mpv 0.41.0 源码（`meson.build`：`libplacebo = dependency('libplacebo', ...)`，`features['libplacebo'] = true` 硬编码）确认 **libplacebo 在 0.41 上根本没有 `-Dlibplacebo` 选项可关**——不是"默认开启可以关掉"，是选项被移除了，唯一去掉它的办法是整体换回 mpv 0.36+ffmpeg 6.0（v6 线），这是构建链重构级别的工作，2026-08-11 决定暂不做，接受 v9 线 10.15MiB 现状。

　　顺带查证 gpu-next（libplacebo 带来的新渲染管线）对 mova **结构性用不上**：media_kit 的 iOS 渲染（`media_kit_video/ios/.../TextureHW.swift`）走的是 mpv **render API**（`mpv_render_context_create`），而 render API 目前只支持传统 `vo=gpu`，加 gpu-next 支持的 PR（mpv-player/mpv#16818）还是 draft、被 maintainer 评价设计有问题，没有合并时间表——即便重新接入 v6 线也不会损失 mova 实际用得上的能力，libplacebo 目前对 mova 纯粹是死重，只是暂时技术上去不掉。继续瘦身见下方"Darwin movaslim flavor"一节 |
