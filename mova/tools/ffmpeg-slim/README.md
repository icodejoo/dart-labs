...
| iOS（ios-arm64 only） | 🟡 CI 构建通过，体积优化进行中，**未接入 mova 工程、未做任何真机播放验证** | 走 `media-kit/libmpv-darwin-build`（iOS/macOS 共用一个仓库），Nix + 固定版本 Xcode，`movaslim` flavor（ffmpeg n9.0 + mpv v0.41.0 + libplacebo/OpenGL，硬解 VideoToolbox，解码器清单照搬 Android）。2026-08-11：给 `cross-files/ios-arm64.ini` 补上 Android 上收益最大的编译期瘦身 flag（`-fvisibility=hidden`/`-ffunction-sections -fdata-sections`/`-Wl,-dead_strip`，之前因 ffmpeg configure 报 `libxml-2.0 not found via pkg-config` 回退过，根因已用 PKG_CONFIG_PATH wrapper 修好），CI run 31454544613 验证通过：strip 后体积从 12,374,408 B 降到 10,967,592 B（≈10.46MiB，−11.4%）。接着又给 ffmpeg 自己的 configure 加 `--enable-lto`、cross-file 加 `-flto`（Android 全链路 LTO 又省了 −521KB），CI run 31456135129 验证通过，无 LTO 相关报错：再降到 10,666,384 B（≈10.17MiB，累计 −13.8%）。研究确认 libplacebo 自己的 `src/meson.build`（`needs_spirv = vulkan or d3d11`，两者都 disabled）证明 glslang/shaderc 对纯 OpenGL 构建是死重，于是把 `-Dglslang=enabled` 改成 `disabled`（顺手关了本来就没接上的 lcms/dovi/libdovi/xxhash/unwind）——CI run 31457206131 验证通过，但体积只降了 **16,384 B（10,650,000 B，−0.15%）**：`__text` 段几乎没变，说明前一轮 LTO+dead_strip 已经把这块死代码吃干净了，这步的价值是"少一个依赖、CI 更快"而不是省体积。**libplacebo/glslang 配置项这条路基本到底了**，剩下跟 Android 6.52MiB 的差距大概率是这两个库本身的静态代码/C++ 模板膨胀，不是配置能抠出来的，继续瘦身见下方"Darwin movaslim flavor"一节 |
