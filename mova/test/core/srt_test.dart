...
import 'package:mova/src/core/stt/srt.dart';

void main() {
  group('formatSrt', () {
    test('formats a single cue with 1-based index and comma milliseconds', () {
      final srt = formatSrt([
        const MovaSttCue(text: '大家好', start: Duration(seconds: 1), end: Duration(seconds: 3, milliseconds: 500)),
      ]);
    ...
    test('formats multiple cues with sequential indices', () {
      final srt = formatSrt([
        const MovaSttCue(text: 'a', start: Duration.zero, end: Duration(seconds: 1)),
        const MovaSttCue(text: 'b', start: Duration(seconds: 1), end: Duration(seconds: 2)),
      ]);
  ...
  group('parseSrt', () {
    ...
    test('round-trips through formatSrt', () {
      final original = [
        const MovaSttCue(text: '第一句', start: Duration.zero, end: Duration(seconds: 2)),
        const MovaSttCue(text: '第二句\n带换行', start: Duration(seconds: 2), end: Duration(seconds: 4)),
      ];
