...
import 'package:mova/src/core/stt/subtitle_store.dart';

void main() {
  late Directory tempDir;

  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('mova_stt_subtitle_test_');
  });
  ...
  test('load returns null when nothing is cached for this source', () async {
    final store = MovaFileSttSubStore(dir: FixedSttSubtitleDirProvider(tempDir.path));
  ...
  test('save overwrites a previous entry for the same source', () async {
    ...
    await store.save('https://host/a.mp4', [
      const MovaSttCue(text: 'new', start: Duration.zero, end: Duration(seconds: 1)),
    ]);
