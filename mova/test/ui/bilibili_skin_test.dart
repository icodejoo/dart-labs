...
void main() {
  ...
  test('the danmaku track is tagged MovaSlot.overlay', () {
    final skin = MovaBilibiliSkin();
    final track = skin.components().firstWhere((c) => c.name == 'danmakuTrack');
    expect(track.slot, MovaSlot.overlay);
  });

  testWidgets('renders bilibili chrome end to end when danmaku is enabled', (t) async {
    final api = FakeMovaApi(options: const MovaOpts(danmaku: MovaDanmakuConfig(enabled: true)));
