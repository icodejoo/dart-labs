# mova 0.3.0 架构设计（插件化 UI：Plugin / Component / Skin 三层契约）
...
- 组件已经持有 `api`（经 `build(context, api, children)` 传入）、已经用 `MovaSelect` 自渲染，
  但没有一个**统一的能力契约**说清"一个组件能拿到哪两种能力、边界在哪"。
- "谁决定显隐"目前散在三处：`components(MovaState)` 按 source 类型换 VOD/直播底栏、
  `assemble()` 里的 `_PipHidden`/`_LockedHidden`/`_BarVisibility` 包整个操作层、
  组件内部各自 `MovaSelect` 切图标。三者其实是**三种不同粒度**的职责，但没写清边界，
...
- 不改 core↔ui 的通信模型（ui→core 直接调 `MovaApi`，core→ui 走事件流，维持非对称，理由见 §2）。
...
- **ui → core**：ui 持 `MovaApi` 抽象接口**直接调方法**（`play()`/`seek()`/`setLocked()`…），
  不经 emitter 转发命令。

**为什么不做"双向都走 emitter"的命令总线**：ui→core 方向本就不需要解耦——ui 依赖的是
`MovaApi` 抽象（测试用 `FakeMovaApi` 替身即证），已经"依赖抽象不依赖实现"。把方法调用改成
...
> 插件的写操作**必须**经 `MovaApi` 这一层（`play`/`seek`/…），不得绕到 `MovaEngine`/`MovaKernel`
> 内部，否则会绕过 `MovaHook`（鉴权/边界改写/错误上报）。这条写进 `MovaPlugin` 契约文档。
...
│   MovaDefSkin（三层骨架，assemble 可覆写） / 自定义 MovaSkin   │
├─────────────────────────────────────────────────────────────┤
│ Component   功能单元：一个能力=一个组件（start/pause/全屏/字幕）│
│   with MovaPlugin，自渲染、插槽无关                              │
├─────────────────────────────────────────────────────────────┤
│ MovaPlugin    纯能力 mixin，与业务无关，只给两样东西：           │
│   ① 持有 MovaApi（能发指令）  ② 订阅 emitter（能听事件）         │
└─────────────────────────────────────────────────────────────┘
```
...
## 4. MovaPlugin —— 纯能力 mixin

### 4.1 它只负责两件事

1. **持有 `MovaApi`**：让组件能发指令（读 `api.state`、调 `api.play()`…）。
...
- **Stateless 组件（默认，纯渲染型）**：
  - 「持有 api」→ 经 `MovaScope.of(context)` 或 `build(context, api, …)` 参数拿到，已有。
  - 「订阅」→ 经 `MovaSelect`/`MovaProgSelect`/`MovaUiSelect` 声明式订阅（内部 `StreamBuilder`
    + `.distinct()`，广播流上重订阅安全）。**这类组件不需要 `with MovaPlugin`**——两种能力
    已由 scope + selector 结构性地提供。
- **Stateful 组件（少数，事件副作用型）**：需要在 `initState` 建订阅、`dispose` 退订阅，
  这才是 `MovaPlugin` mixin 真正落地的地方：
...
mixin MovaPlugin<T extends StatefulWidget> on State<T> {
  /// The capability surface. 能力面。
  MovaApi get api => MovaScope.of(context);
...
保持现有 `MovaComp`（`name`/`slot`/`order`/`children`/`build`），**补两条新规矩**：
...
| **组件内渲染**：单个组件跟状态变外观 | 播放键切图标、进度条走位、角标闪现 | **Component 自己** | `MovaSelect` 选字段重建 |

**边界只剩两家**：整层显隐归骨架（做一次，对应你"整体隐藏、不要每个元素各自隐藏"的要求），
其余一切——粗粒度成员 + 组件内渲染——都归组件自己（`MovaSelect`）。层级门控（pip/锁定/闲置）
...
组件默认 **Stateless + `MovaSelect`**，仅"事件副作用型"用 **Stateful + `MovaPlugin`**：

| 监听类型 | 例子 | 载体 | 能力来源 |
|---|---|---|---|
| **渲染响应**：状态变→重画 | 播放键切图标、进度条走位、锁定切显隐 | Stateless | `MovaScope` + `MovaSelect` |
| **副作用响应**：事件到→干件事（非重画） | 收到 `MovaError` 弹提示、`MovaLiveEdgeReach` 闪角标、进 pip 触发系统调用 | Stateful | `with MovaPlugin` |

**不用裸 `StreamBuilder`**（除非像 `MovaSelect` 那样封装并配 `.distinct()`）：它无选择性重建
（一有事件重建整棵子树，position 200ms 一跳会引发重建风暴），且 build 里现算 stream 会踩
重订阅陷阱。现有 `MovaSelect` 已经把这层封好（广播流 + `.distinct()`），组件复用它即可。

---

## 7. Skin —— 变薄的组装者

`MovaSkin` 接口**保留但变薄**，`MovaDefSkin` 从"焊死实现"改为"**可分档覆写**"，给出三档灵活度：

### 7.1 三档定制

| 档 | 做法 | 能改什么 | 不用碰什么 |
|---|---|---|---|
| **① 补丁档** | `MovaDefSkin(patches: [...])` | 增/删/替换/重写组件、在**已有插槽间**挪位置、改颜色尺寸（`MovaTheme`） | 不写布局代码 |
| **② 半覆写档** | `extends MovaDefSkin` 只 override 某一层的受保护方法（`buildOperableLayer` 等） | 重排版（把控制条挪顶部、侧栏、画面内浮层） | 复用默认组件集、主题、补丁能力、其余各层 |
| **③ 全实现档** | `implements MovaSkin` | 布局、交互、组件全自定义 | —— |

**关键改动**：把 `MovaDefSkin.assemble()` 内部的三层拆成**可单独覆写的受保护方法**
...
`MovaDefSkin` 的骨架就是你定的三层，写死为内置皮肤的"统一整体"：
...
| xgplayer | mova 取舍 |
|---|---|
| EventEmitter 核心 | ✅ 已有（`MovaBus` 四条流），core→ui 走它 |
| Plugin 有生命周期、可运行时 `install/uninstall` | ❌ 不做动态增删。定制在**构造皮肤时**静态组装（`patches`/覆写），够用且简单 |
| 内置控件与第三方用同一套注册机制 | ✅ 认同并对齐——内置组件也走 `MovaComp`+插槽，无"皮肤特权通道" |
| 插件间不互相引用、都过 player 事件 | ✅ 已是（组件只认 `MovaApi`+事件流，互不引用） |
| CSS 皮肤 | → 组合装配（`assemble` + `MovaTheme`） |
...
**决策（2026-07-31）**：`MovaGestConfig` 不再用 `leftVerticalVolume: bool` 这种把侧别与动作
...
///（如把亮度放右侧、或禁用某一侧）。
enum MovaGestAction { volume, brightness, seek, none }

MovaGestConfig(
  leftVertical: MovaGestAction.brightness,  // 默认：左亮度
  rightVertical: MovaGestAction.volume,     // 默认：右音量
  horizontal: MovaGestAction.seek,
  doubleTapSeek: true,
  pinchZoom: true,
)
```
...
组件注释、`README.md`、`.claude` 下 `mova-dev` skill 里的"左音量/右亮度…刻意为之…勿修正"。

**HUD 反馈维持居中**（overlay/center），不分左右——同样对齐主流（bilibili 音量/亮度弹居中胶囊）。

### 9.2 MovaSlot 新增 left / right
...
- **MovaPlugin**：新增一个 mixin 文件（`ui/scope/plugin.dart` 量级），~30 行。现有 stateless
  组件**零改动**（它们本就靠 scope+selector）。只有真正做副作用的组件改成 `with MovaPlugin`。
...
- **测试**：新增 `MovaPlugin` 单测（订阅 dispose 回收）；手势翻转的现有测试断言要跟着改。
...
- ✅ **`MovaGestConfig` → 侧别→动作映射**（`leftVertical: MovaGestAction.brightness`），
  侧别与动作解耦。见 §9.1。
...
1. 加 `MovaSlot.left/right` + `buildSlots`/`assemble` 支持（不改行为，先铺位置）。
2. 加 `MovaPlugin` mixin + 单测；挑一个副作用组件（如 error toast）做 pilot 迁移验证。
3. `MovaDefSkin.assemble` 拆成三个受保护方法 + ②档覆写的 widget 测试。
