...
**决策依据：** 2026-08-04 与 playora 对比后，用户选定三个全做（先播放列表+下一集，再广告）。mova
...
**现有可复用原语（读代码确认）：**
- `MovaSlot.overlay`（`ui/slots/slot.dart`）——不受锁定/自动隐藏门控，弹幕已用。
- `MovaComp` + `MovaSelect` / `MovaPlugin`（`ui/`）——出组件、按状态重建、副作用订阅。
- `MovaHook{beforeOpen,beforeSeek,beforePlay,onError}` + `MovaHookChain`（`core/interceptor/`）。
- 事件：`MovaDone` / `MovaSourceChg` / `MovaDurChg`（`core/events/events.dart`）；progress 流带 position。
- `MovaOpts` 分节 + `copyWith`（`core/options/options.dart`）——新增配置各占一节。
...
- [x] **A1 模型 + 配置**（2026-08-04）：`core/model/playlist.dart` 的 `MovaPlistItem{source, title?, subtitle?,
  poster?}`（附 `displayTitle` getter）；`core/options/playlist_config.dart` 的 `MovaPlistConfig{enabled=false,
  items=const[], initialIndex=0, autoPlayNext=true, nextUpLeadTime=Duration(seconds:10)}`。接进 `MovaOpts`
  新节 + `copyWith` + `==/hashCode` + `options.dart` export/import。**`onItemChanged` 回调改为控制器的
  `indexChanges` 流**——放配置里会破坏 `==` 值语义，用流更干净。
- [x] **A2 控制器**（2026-08-04）：`core/playlist/playlist_controller.dart` 的 `MovaPlistCtrl`——持当前
  下标、`next()`/`previous()`/`jumpTo(i)` 调 `api.open()`，`hasNext`/`hasPrevious`/`currentItem`/`nextItem`/
  `previousItem` getter + `indexChanges` 流。纯 Dart（无 Flutter，守 `purity_test`），注入 `MovaApi`。
...
- [x] **B1 模型 + 配置**（2026-08-04）：`core/model/ad.dart` 的 `MovaAdBreakKind{pre,mid,post}` + `MovaAdBreak
  ...
  {started,completed,skipped,clicked}` + `MovaAdEvent{type, adBreak}`；`core/options/ad_config.dart` 的
  `MovaAdConfig{enabled=false, breaks=const[], onAdEvent?}`。接进 `MovaOpts` 新节。
- [x] **B2 广告控制器**（2026-08-04）：`core/ad/ad_controller.dart` 的 `MovaAdCtrl`——**改为宿主驱动的
  编排器，不做 `MovaHook`**（拦截器在 engine 构造期注入、拿不到 api 来切源，做编排器更顺、与
  `MovaPlistCtrl` 一致）。`load(content)` 起播（有 pre 先播 pre）；订阅 progress 触发 mid（保存正片
  位置，广告后 `_playContent(at:)` 续播，靠 seek 寄存落地）；`MovaDone` 驱动 ad→content / content→post→idle
...
- [x] **B4 副作用抑制**（2026-08-04）：广告播放期间抑制正片侧 STT。给 `MovaSttApi` 加只读 `isRunning`
  getter（`MovaSttSvc` 转发 `_started`，`attach()` 不重置它，故 STT 会带着跑进广告——正是要治的）；
  `MovaAdCtrl._playAd` 捕获 `_sttWasRunning = api.stt.isRunning` 并 `stop()`，`_playContent` 续播后
  **仅当广告前在跑时**才 `start()`。单测覆盖：广告期间抑制、续播后恢复、广告前未开则不误开。
- [x] **B4+ 运行时任意位置插入**（2026-08-04，用户追加需求"广告要支持在任意位置插入"）：
  `MovaAdCtrl.playAdNow(MovaAdBreak)`——在当前正片位置即时插播、播完回到打断处（`_lastContentPosition`
...
1. **playlist ↔ ads 组合争抢 `MovaDone`**：`MovaAdCtrl` 新增 `contentEnded` 流（正片+后贴片全播完才触发），两个控制器类文档写明组合契约——组合时 `autoPlayNext: false` + 由 `contentEnded` 驱动 `next()`。
...
6. **`MovaPlistCtrl.jumpTo`**：先 `await open` 再提交下标/发 `indexChanges`，open 失败时下标不前移。
...
1. **控制器归属**：`MovaPlistCtrl`/`MovaAdCtrl` 是宿主 `new` 出来注入，还是 `MovaApi` 暴露 getter？
