# mova 0.2.0 阶段 C：直播时移（含阶段 D 收尾发布） — 实现计划
...
`createMovaEngine()`，接好了 brightness/PiP/orientation 三个真实端口——这与阶段 B/C 谁先落地
...
| 已有 | 位置 | 状态 |
|---|---|---|
| `MovaLiveSeekMode{off,dvr,timeshift}` | `core/options/live_config.dart` | 完整 |
| `MovaLiveConfig.dvrWindow` / `edgeThreshold`（默认 10s） | 同上 | 完整 |
| `MovaState.liveSeekable` / `seekableWindow` / `timeshiftBehind` | `core/state/state.dart` | 字段与 `copyWith(clearTimeshift:)` 齐备；**`timeshiftBehind` 至今无人写入** |
...
| `MovaTimeShiftChg(behind)` / `MovaLiveEdgeReach` 事件类 | `core/events/events.dart` | 类已定义；`MovaLiveEdgeReach` 仅被占位实现发过，`MovaTimeShiftChg` 从未被发过 |
| `MovaStrs.live` / `backToLive` / `timeshift` | `core/options/strings.dart` | 字段已在，`timeshift`/`backToLive` 尚无人使用 |
| `LiveBarComponent{liveBadge, backToEdge}` | `ui/components/live_bar.dart` | 存在；`backToEdge` 现在调的是 `api.reload()` |

**唯一的占位实现**：`MovaEngine.backToLiveEdge()` 目前是 `reload()` + 发 `MovaLiveEdgeReach`
...
| §4.4 `MovaState` 无 `sourceTitle` | 有 | 有 |
...
- 硬编码文案走 `MovaStrs`、颜色/尺寸走 `MovaTheme`，组件里不许出现字面量中文或
  `Colors.*`/`Color(0x...)`。
...
`ios/mova.podspec`、`example/lib/main.dart`、`README.md`、`CHANGELOG.md`、`doc/SPEC.md`、`CLAUDE.md`。
...
## Task 1: `MovaLiveConfig` 扩展（DESIGN §6.2 的四个开放点）
...
**Interfaces:**
- Consumes: `MovaState`（`core/state/state.dart`，已存在）
- Produces:
  ...
  - `MovaLiveConfig` 新增字段 `MovaTimeShiftBldr? urlBuilder`、`MovaBackToLive? backToLive`、
    `bool autoBackToLiveOnStall = false`、`MovaLiveWindowSolver? windowResolver`
...
Expected: FAIL — `The getter 'urlBuilder' isn't defined for the class 'MovaLiveConfig'`

- [x] **Step 3: 实现 `lib/src/core/options/live_config.dart`**

文件顶部补 import 与两个 typedef、一个枚举（放在 `MovaLiveSeekMode` 之后、`MovaLiveConfig` 之前）：
...
`MovaLiveConfig` 类内，在 `edgeThreshold` 字段之后追加四个字段：
...
```bash
git add -A
git commit -m "feat(mova): add timeshift url builder, back-to-live strategy and window resolver knobs"
```
...
**Interfaces:**
- Consumes: `MovaState`（已存在）、`MovaLiveConfig` / `MovaLiveWindowSolver`（Task 1）
- Produces:
...
`lib/mova.dart` 在 `export 'src/core/interceptor/interceptor.dart';` 之后按字母序插入：
...
```bash
git add -A
git commit -m "feat(mova): add pure timeshift window/behind/edge helpers"
```
...
发 `MovaTimeShiftChg` / `MovaLiveEdgeReach`。
...
**Interfaces:**
- Consumes: `resolveWindow` / `behindOf`（Task 2）、`MovaTimeShiftChg` / `MovaLiveEdgeReach`（已存在）
- Produces: 无新公开类型；`MovaState.timeshiftBehind` 首次被写入
...
```bash
git add -A
git commit -m "feat(mova): reduce live timeshift lag into MovaState and announce edge transitions"
```
...
**Interfaces:**
- Consumes: `MovaTimeShiftBldr`（Task 1）、`MovaTimeShiftChg` / `MovaLiveEdgeReach`（已存在）
...
```bash
git add -A
git commit -m "feat(mova): implement timeshift seeking by reopening at a host-built url"
```
...
`MovaSourceChg`/`MovaReady`、重跑拦截链。回边缘只是换个播放位置，不该让 UI 以为换了源。
...
**Interfaces:**
- Consumes: `MovaBackToLive` / `MovaLiveConfig.effectiveBackToLive` / `autoBackToLiveOnStall`（Task 1）
...
```bash
git add -A
git commit -m "feat(mova): make backToLiveEdge strategy-driven and add stall auto-recovery"
```
...
**Interfaces:**
- Consumes: `MovaState.liveSeekable` / `seekableWindow` / `timeshiftBehind`、`MovaStrs.live` /
...
import 'package:mova/src/core/state/state.dart';
import 'package:mova/src/ui/components/live_bar.dart';
...
void main() {
  ...
  testWidgets('the badge reads LIVE at the edge and 时移 while replaying', (t) async {
    ...
    api.push(_behind);
    await t.pump();
    // A second pump settles a Stream.multi replay hop from MovaBus — see the
    // identical note on the preview-bubble tests in phase B.
    //
    // 第二次 pump 用来落定 MovaBus 的 Stream.multi 重放跳转——与阶段 B 预览气泡
    // 测试里的说明相同。
    await t.pump();
...
顶部还要 `import 'package:mova/src/core/options/options.dart';`（`MovaOpts`/`MovaStrs`）。
...
/// `MovaState.liveSeekable`, so a non-DVR stream keeps 0.1.0's badge-only bar.
///
/// 直播底部控制条组合组件。
///
/// 布局对应 DESIGN §5.4 的直播树
/// `bottomBar/{liveBadge, seekBar, timeshift, backToLive}`。进度条仅在
/// [seekable] 为真时才被**放置**；皮肤依据 `MovaState.liveSeekable` 决定该值，
/// 因此非 DVR 流保持 0.1.0 那种只有角标的底栏。
class LiveBarComponent extends MovaComp {
...
```bash
git add -A
git commit -m "feat(mova): add seek bar, timeshift indicator and back-to-live button to the live bar"
```
...
若顶部缺 import，补 `package:mova/src/core/model/source.dart`、
`package:mova/src/core/options/options.dart`、`package:mova/src/core/state/state.dart`。
...
```bash
git add -A
git commit -m "test(mova): pin live-seek gesture gating for dvr streams"
```
...
- Modify: `lib/src/core/state/state.dart`, `lib/src/core/api.dart`, `lib/src/core/engine.dart`,
  `lib/src/ui/components/top_bar.dart`, `lib/mova.dart`, `test/support/fake_api.dart`
...
`test/ui/top_bar_test.dart`：**删掉**第 24–45 行那一整块（`// NOTE: MovaApi exposes no
...
Expected: FAIL — `The named parameter 'pipSupported' isn't defined`（`MovaState.copyWith`）、
`The getter 'pipSupported' isn't defined for the class 'MovaEngine'`

- [x] **Step 4: 加 `MovaState.pipSupported`**
...
```dart
  ...
  /// per engine from `MovaPipPort.isSupported()`; `false` until that probe
  ...
  /// `MovaPipPort.isSupported()` 探测一次；探测返回前为 `false`，因此依赖它隐藏
...
- [x] **Step 5: 加 `MovaApi.pipSupported` 并在 engine 探测**
...
```dart
  ...
  ///
  /// A synchronous mirror of [MovaState.pipSupported] so components can branch
  /// without a selector. Components that must *rebuild* when it flips should
  /// still watch `MovaState.pipSupported` through a `MovaSelect`, since the
  /// value is resolved asynchronously shortly after construction.
  ///
  /// 当前平台是否支持系统画中画。
  ///
  /// [MovaState.pipSupported] 的同步镜像，便于组件不用 selector 也能分支。需要在
  /// 它翻转时**重建**的组件仍应通过 `MovaSelect` 观察 `MovaState.pipSupported`，
  /// 因为该值是在构造后不久异步解析出来的。
...
1. `_state` 初值改为 `MovaBus<MovaState>(const MovaState(pipSupported: true))`（保持既有"默认支持"语义）。
2. 把字段 `bool pipSupported = true;` 及其上方注释替换为一对 getter/setter，使能力 getter 与
   状态上的 `MovaSelect` 永远一致：
...
`lib/mova.dart` 末尾（`export 'src/ui/slots/tree.dart';` 之后）追加：
...
只是这三个具体适配器类自身的导出，方便宿主想绕开 `createMovaEngine()` 自己拼装时能直接
`import 'package:mova/mova.dart'` 拿到它们。`example/lib/main.dart` 已经在用
`createMovaEngine()`（真实的 brightness/PiP/orientation 三个端口已经装好），本任务不需要
再碰它。
...
```bash
git add -A
git commit -m "feat(mova): add a synchronous pip capability flag and hide the pip button without it"
```
...
grep -rn "Duration(seconds: 10)" lib/src/core/live lib/src/core/engine.dart ; echo "--- 上面应为空：边缘阈值必须来自 MovaLiveConfig ---"
...
```bash
git add -A
git commit -m "test(mova): audit the live timeshift openness contract"
```
...
Expected: `version: 0.2.0`、`homepage: https://github.com/icodejoo/dart-labs/tree/main/mova`

- [x] **Step 2: 改写 podspec 元数据**

`ios/mova.podspec` 前段替换为（保留后面的 `source_files` / `dependency` / `platform` /
...
scrub-preview thumbnails, live timeshift, and Android picture-in-picture.
                       DESC
  s.homepage         = 'https://github.com/icodejoo/dart-labs/tree/main/mova'
...
若本机有 CocoaPods：`cd ios && pod lib lint mova.podspec --allow-warnings`；
...
```bash
git add -A
git commit -m "chore(mova): replace ios podspec template placeholders with real metadata"
```
...
**Interfaces:**
- Consumes: `MovaSource(type: MovaStreamType.live)`、`MovaOpts(live: MovaLiveConfig(...))`、
  `MovaLiveSeekMode`、`MovaTimeShiftBldr`
- Produces: example 里五个入口（原三个 + 直播 + 时移），每个 demo 带自己的 `MovaOpts`
...
`createMovaEngine()`（`_engine = createMovaEngine();`，装好了真实的 brightness/PiP/orientation
...
```dart
  ...
  void initState() {
    super.initState();
    _engine = createMovaEngine(options: _demos[_index].options);
  ...
  Future<void> _switch(int i) async {
    ...
    setState(() {
      _index = i;
      _engine = createMovaEngine(options: _demos[i].options);
    });
    await old.dispose();
...
`late final MovaEngine _engine;` 改为 `late MovaEngine _engine;`（现在会被重新赋值）。

> 若阶段 B 已落地，`createMovaEngine()` 还会多接好预览相关的端口（缩略图目录/抽帧器），
...
```bash
git add -A
git commit -m "feat(mova): add live and timeshift demos to the example app"
```
...
- 「关键 API 与不变式」里把「`MovaApi.seek()` 在直播时被引擎忽略」改成
...
```bash
git add -A
git commit -m "docs(mova): document live timeshift and reconcile design doc with the implementation"
```
...
```bash
git add -A
git commit -m "chore(mova): record 0.2.0 release pre-flight results"
```
...
10. 直播 DVR 入口：底栏出进度条与红色 `LIVE`；拖动后角标变灰 `时移` 且出现 `-MM:SS`；
    点回直播按钮回到边缘、角标变红。
11. 时移入口：**只验证 UI 与事件**——拖动后地址确实变了（可加临时日志或看 `MovaTimeShiftChg`），
...
```bash
git add -A
git commit -m "docs(mova): record 0.2.0 real-device verification results"
```
...
5. 阶段 C **未新增任何依赖**。
6. `ios/mova.podspec` 的 `version` 与 `pubspec.yaml` 一致，无模板占位字段。
