...
> 想接什么后端（云端/自建模型/自己的 MCP 调用）都行，但那是宿主的选择，不是 mova
...
| macOS | 与 iOS **同一套** Apple Speech Framework | 同上 | ⚠️ **mova 目前没有 macOS 原生插件**（`macos/` 目录不存在）——要从零搭一个平台插件骨架，比"加个 case"重，但 STT 逻辑可照抄 iOS 那份 |
...
- **不追求"四端行为完全一致"**：这是本次调整对既有价值取向的一次明确背离——mova
...
- **`MovaApi` 面**（若走 Flutter 侧方案）：预计新增 `MovaApi.subtitles`（字幕流，同 `preview`
  的形状）+ `MovaSubChg` 一类事件，具体签名留到实现阶段确定。
...
- **技术方向**：独立解码路径（仿阶段 B）+ **平台原生 STT 优先、否则关闭** + 通用
  `MovaSttEngine` 注入口子（非 MCP 专用）+ 字幕叠层组件。
