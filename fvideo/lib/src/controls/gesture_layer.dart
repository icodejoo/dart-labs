...
/// Side mapping honors [FvideoGestureConfig] (left=volume, right=brightness).
///
/// 视频画面的原始手势识别层。
///
/// 只发出高层*意图*（进度/音量/亮度/缩放/点击），不直接操作播放状态，
/// 因此与具体 UI、控制器解耦。侧别映射遵循 [FvideoGestureConfig]（左音量/右亮度）。
class FvideoGestureDetector extends StatefulWidget {
  /// Gesture enable/side config.
  ///
  /// 手势开关与侧别配置。
  final FvideoGestureConfig config;
  ...
  @override
  State<FvideoGestureDetector> createState() => _FvideoGestureDetectorState();
}

/// State machine for [FvideoGestureDetector]; holds per-drag snapshots.
///
/// [FvideoGestureDetector] 的状态机；保存每次拖动的起始快照。
class _FvideoGestureDetectorState extends State<FvideoGestureDetector> {
