...
在 fvideo 子工程内工作时的指引。详见 [doc/PRD.md](doc/PRD.md)（需求/决策）、
...
`dart-labs` 是 monorepo，fvideo 是子目录。`origin` 同时推 codeup 与 github(icodejoo/dart-labs)。
提交信息用 `type(scope): message`，scope 用 `fvideo`。装了 lefthook 钩子（本机若无 lefthook 会跳过）。
