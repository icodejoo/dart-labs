...
void main() {
  group('yml lenient config — enabling', () {
    test('enables a type for a class with no annotation at all', () {
      ...
      expect(out, contains("const LenientIntConverter().fromJson(json['count'])"));
    });

    test('leaves a type alone when the yml disables it and nothing else opts in', () {
      final out = debugApplyLenientRewrite(
        sourceFor({'count': intField}),
        lenientOptions: {
          'int': {'enabled': false},
    ...
    test('yml overrides a class-level annotation that turned the type off', () {
      ...
      expect(out, contains("const LenientDoubleConverter().fromJson(json['score'])"));
    });
    ...
    test('a class-level annotation still applies to types the yml never mentions', () {
      final out = debugApplyLenientRewrite(
      ...
      expect(out, contains("const LenientBoolConverter().fromJson(json['active'])"));
    });
  });
  ...
  group('yml lenient config — default values', () {
    test('yml defaultValue is threaded into the converter call', () {
      ...
      expect(out, contains("const LenientIntConverter().fromJson(json['count'], 5)"));
    });
    ...
    test('a field @JsonKey(defaultValue:) beats the yml default', () {
      ...
      expect(out, contains("const LenientIntConverter().fromJson(json['count'], 9)"));
      expect(out, isNot(contains(', 5)')));
    });

    test('yml default also replaces the plain auto-default when leniency is off', () {
      final out = debugApplyLenientRewrite(
        sourceFor({
          'count': intField,
          'score': doubleField,
    ...
    test('string defaults are escaped into a valid Dart literal', () {
      ...
      expect(out, contains(r"json['label'] as String? ?? 'it\'s a \\ \$tricky one'"));
    });
    ...
    test('bool default renders as a bool literal', () {
      ...
      expect(out, contains("const LenientBoolConverter().fromJson(json['active'], true)"));
    });
    ...
    test('Map keeps its const {} fallback — the yml has no Map key', () {
      ...
      expect(out, contains("Map<String, int>.from(json['extras'] as Map? ?? const {})"));
    });
  });
  ...
  group('yml lenient config — dateTime', () {
    ...
    test('yml utc overrides the class annotation but not a field annotation', () {
  ...
  group('yml lenient config — lenient parsing', () {
    ...
    test('a mistyped enabled/defaultValue is skipped rather than thrown', () {
      ...
      expect(out, contains("const LenientStringConverter().fromJson(json['label'])"));
    });
  });
}
