...
**run 35206555833（2026-09-17）跑绿，产物只有一个 `libmpv.dylib`，10,420,552 字节 ≈ 9.94MiB**——比 movaslim 动态版（10.76MiB/13 文件）体积相近但是单文件了，跟 Android 的 6.52MiB 单文件终于是同一种可比形态（差距约 53%，很可能是 Android flavor 脚本还上了 `-Os`/`-fvisibility=hidden`/`--gc-sections`/跨库 LTO 这类编译器级瘦身,movaslim 目前只做了 decoder/demuxer 裁剪 + strip，编译器级优化是下一步）。`flavors-mova-slim-ios.sh` 本体标注为已被此 patch 取代的历史记录，不再删除。checkout 未锁定 `media-kit/libmpv-darwin-build` 的 ref（用的是当时最新 main），后续应视情况锁定。**曾有 `mova-libmpv-winbuild-zhangfly` 分支走 ffmpeg n9.0+mpv 0.41（v9 线）做过 CI 验证（10.15MiB），该分支已作废不用**，不可比、不要复用 |
