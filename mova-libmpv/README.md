...
| iOS | ✅ **2026-09-17 v3 首次真实 CI 跑绿**（run 35194396206），其他平台 job 暂时全部 `if: false` 只留 iOS 快速迭代 | 原 `ios` job 那套从零手写的 ffmpeg+8 依赖库 meson 交叉构建 + mpv（v2 线，`flavors-mova-slim-ios.sh`，6.52MiB 那套定稿数字是本地/CI 均从未真正跑绿过的理论值）已废弃、**未删代码但整段被新 job 替换**。改为复用 `darwin` job 同一套上游仓库/工具链（`media-kit/libmpv-darwin-build`，Nix + 固定版本 Xcode），只跑一个精确 flake target（iOS 设备 arm64、libs 格式、video variant、**default flavor**，不含 simulator/macOS）。**中间踩了两个坑**：① TARGET 属性名前缀取的是 `nix/packages/` 下的目录名 `mk-out-archive`，不是包内部 `name = "archive"` 那个展示用变量，第一次真跑（35193860905）因此报 "flake ... does not provide attribute ... archive-libs-ios-arm64-video-default"；② 修完之后 nix 构建本身（ffmpeg+mpv+全部依赖）已经成功，卡在 CI 自己的校验脚本——`result` 是指向 Nix store 的符号链接，`find result -name '*.tar.gz'`（无尾斜杠、无 `-L`）不解引用符号链接，静默找不到文件，而前一行 `ls -la result/`（有尾斜杠）碰巧解引用了，掩盖了这个问题；改成 `find -L` 后（run 35194396206）全绿。**产物形态需要注意**：`libs` 格式解出来是 18 个独立 `.dylib`（libmpv.dylib 本体仅 1,890,312 字节 ≈1.80MiB，其余全是 ffmpeg/dav1d/libass/freetype/harfbuzz/fribidi/mbedtls/libxml2/libpng/uchardet 各自独立的动态库，总计远大于本体），**不是** v2 线追求的"单一 libmpv.dylib 静态链接全部依赖"那种形态——这是 upstream 默认 flavor 的产物结构，mova 接入构建产物、以及后续瘦身叠加时都要考虑这一点（要么切 `xcframeworks` 格式、要么自定义 flavor 改成静态链接，尚未决定，是后续任务）。`flavors-mova-slim-ios.sh` 里 mova 自己的裁剪清单先搁置，等这个 default-flavor 验证通过后再作为后续任务叠加上去（同 Android 当年"先接 media-kit/libmpv-android-video-build 跑通，再上自定义 flavor patch"的顺序）。checkout 未锁定 `media-kit/libmpv-darwin-build` 的 ref（用的是当时最新 main），后续应视情况锁定。**曾有 `mova-libmpv-winbuild-zhangfly` 分支走 ffmpeg n9.0+mpv 0.41（v9 线）做过 CI 验证（10.15MiB），该分支已作废不用**，不可比、不要复用 |
