...
**2026-09-18**：macOS/iOS movaslim 验证告一段落（见下表 macOS 行），此前为让出 CI 资源专供
iOS/macOS 迭代而 `if: false` 暂停的 `android-arm64`/`android-other-abi`/`linux`/`windows`
四个 job 已同批重新 `if: true` 启用，与 `darwin`/`ios` 一起跑。

| 平台 | 状态 | 说明 |
|---|---|---|
| Android arm64-v8a | ✅ 已定稿并接入 mova 工程，**真机播放已验证（2026-09-18）** | 6.52MiB（AV1 硬解+软解双通道，2026-08-06 真机复测后改判，CI 复现构建），见下方"定稿结果"。**2026-09-18**：`-fomit-frame-pointer` A/B 后移除（回归，6,190,732→6,050,104 字节，−2.27%），当前 CI 产物已是这个数字；在 STG-AL00 真机（骁龙 bengal，Android 12）上装 mova example app 实测，`syncMovaLibmpv` Gradle task 正确同步了当次 CI 产物到 `jniLibs/`，视频画面正常解码渲染，logcat 无 app 侧 FATAL 崩溃 |
| Android armeabi-v7a / x86_64 | ✅ CI 构建通过，未接入真机验证 | 同一套 flavor 脚本直接复用（`--disable-runtime-cpudetect` 已按架构条件判断，不用改），2026-08-06 CI 矩阵全绿并核验 `dav1d_open` 符号；已接入 `example` 的 jniLibs（`syncMovaLibmpv` Gradle task 自动同步，2026-09-17），还没上真机测过 |
| Android x86 | ✅ **已修复（2026-09-17）** | 根因是共享的 `actions/cache`（"deps" 缓存 key 只按 `ANDROID_LIBMPV_BUILD_REF` 区分，不分架构）里 mpv 自己的 `_build-x86` 目录残留了历史某次不同配置下的 stale 状态，meson 复用旧配置而不是应用当次的 `--default-library shared`，构建出 `libmpv.a`（静态）而非 `libmpv.so`。修复：在 `build.sh` 之前对每个架构强制 `rm -rf deps/mpv/_build-<suffix>`，arm64-v8a job 也同步加了同一道保险。修复后 run 35188876854 验证通过，产物 6,190,732 字节 |
| iOS | ✅ **2026-09-17 audio/protocol 收窄（v3.4）CI 全绿，单文件 6.23MiB——反超 Android 6.52MiB 约 −4.4%** | 原 `ios` job 那套从零手写的 ffmpeg+8 依赖库 meson 交叉构建 + mpv（v2 线，`flavors-mova-slim-ios.sh`，6.52MiB 那套定稿数字是本地/CI 均从未真正跑绿过的理论值）已废弃。v3 改为复用 `darwin` job 同一套上游仓库/工具链（`media-kit/libmpv-darwin-build`，Nix + 固定版本 Xcode），中间踩了两个坑（TARGET 属性名前缀取的是目录名 `mk-out-archive` 不是内部展示变量 `"archive"`；`find` 不解引用 `result/` 符号链接）后于 run 35194396206 首次全绿，用的是**upstream 默认 flavor**，产物是 `libs` 格式解出的 18 个独立 `.dylib`，**总计 18.48MiB**（libmpv.dylib 本体仅 1,890,312 字节 ≈1.80MiB，其余是 ffmpeg/dav1d/libass/freetype/harfbuzz/fribidi/mbedtls/libxml2/libpng/uchardet 各自独立的动态库）。**v3.1（同日，run 35196019964）**：`mova-libmpv/libmpv-darwin-build-mova-slim.patch` 给这套 Nix 构建加一个新 flavor `movaslim`，把 `flavors-mova-slim-ios.sh` 里设计好但一直没接线的 DECODERS/ENCODERS/PARSERS/DEMUXERS/BSFS 清单移植进 `mk-pkg-ffmpeg/meson.build` 的 `video_movaslim_options`，同时 TLS 从 mbedtls 换成 securetransport、去掉 libxml2/uchardet 两个 dylib——**CI 实测 14.21MiB/13 文件**（−23% vs 默认 flavor）。同日追加 strip（`find ... -exec strip -S -x`）：**CI 实测 10.76MiB/13 文件**（run 35196802229，−42% vs 默认 flavor 起点）。**v3.2（同日，写好未验证）**：18 个独立 dylib 里最大的几个（harfbuzz 2.50MiB、avcodec 2.93MiB、libmpv 本体 1.42MiB、freetype 1.09MiB stripped 后）迟迟压不下去的根因是 `mk-out-libs/default.nix` 不管 ffmpeg/dav1d/libass/harfbuzz/fribidi/freetype/libpng 实际怎么链接、无条件把每个包的 `.dylib` 各自拷进最终产物——`libs`/`xcframeworks` 两种产物格式从未把依赖静态合并进一个文件，跨库死代码消除（这是 Android 单文件能小这么多的主因）根本没发生。这轮把 ffmpeg（`--enable-static --disable-shared`，改在 `linking_movaslim_options`）、dav1d/freetype/fribidi/harfbuzz/libpng（meson 原生 `-Ddefault_library=static`，各自 `default.nix` 新增 `flavor` 形参用 `callPackageWith` 的具名自动注入机制透传）、libass（本身是 autotools 包着 meson `external_project` 壳，`-Ddefault_library` 对它不生效，改为新增 `meson.options` 的 `linking` combo 选项 + `meson.build` 里按此选项二选一 `--enable-static/--disable-static`）全部改成 movaslim 下静态构建，`mk-pkg-mpv/default.nix` 新增 `flavor` 形参并透传进它自己内部对 ffmpeg/libass 的 `callPackage`（此前这里完全没传 flavor，mpv 实际链接的 ffmpeg/libass 跟"标称的 movaslim flavor"其实是两份不同的派生，一直没接上），加 `--prefer-static` 兜底；`mk-out-libs/default.nix` 的 `deps` 列表里这些包对 movaslim 不再单独打包（现在应该只剩 `libmpv.dylib` 一个文件，`libpng`/`freetype` 等按依赖关系应被静态折进 mpv/libass/freetype 自身）。**验证过程连环踩坑（同日，5 轮真实 CI 失败后才绿）**——每修一个坑就暴露下一层，都是静态化之前从未被触发过的路径：
① 补丁新增的 `mk-pkg-libass/meson.options` 文件 `git apply` 后是 untracked 状态，Nix 的 `git+file://` flake source 过滤掉未追踪文件，报"not tracked by Git"——CI 那条 `git apply` 步骤本身要加 `git add -A`（这条已在此前一次提交里修，movaslim 补丁本身不用再改）；
② autotools 的 `AC_CHECK_TOOL` 交叉编译时拒绝回退到不带前缀的 `ar`/`ranlib`，静默换成 `false`，libass 静态归档失败报 `libtool: link: false cr ...`（此前只建静态库不需要 ar/ranlib，从未触发）——`mk-pkg-ffmpeg/meson.build` 补齐 `ranlib = find_program('ranlib')` 传 `--ranlib=`，libass 侧在 `configure_options` 里直接加 `AR=`/`RANLIB=` 两个 trailing VAR=value 参数；
③ freetype 静态化后 meson 自动生成的 `freetype2.pc` 把私有依赖 libpng 提升进公开 `Requires:`，libass 消费 freetype2 时因此需要 `libpng.pc` 可解析，但 libass 自己的 Nix 包从未声明 libpng 依赖——给 `mk-pkg-libass/default.nix` 加 `libpng` buildInput；
④ 同理再上一层：mpv 消费 libass.pc 时，libass 静态化后同样把 harfbuzz/fribidi/freetype/libpng 提升进公开 Requires，`mk-pkg-mpv/default.nix` 只有 ffmpeg/uchardet/libass 三个 buildInput——补齐四个；
⑤ libpng 静态化后自己的 `.pc`（这版 libpng 是真两个文件 `libpng16.pc` + 桩文件 `libpng.pc`，不是新版那种真文件+symlink）把 zlib 提升进 Requires，但环境里压根没有 zlib.pc（iOS 系统 libz 靠 `-lz` 走默认链接路径,从不通过 pkg-config）——`mk-pkg-libpng/default.nix` 的 `postFixup` 里对两个 `.pc` 文件都 sed 掉 zlib 那个 token（保留其余 Requires 内容，不是整行删除）；
⑥ 给 mpv 加的 `--prefer-static` 让 meson 的 `dependency('iconv')` 探测失效（已知 meson 在 Darwin 交叉编译下的未解决 bug，mesonbuild/meson#12455，网上搜索确认，非猜测）——mova 精简特性集本来就不需要 iconv，movaslim 下直接 `-Diconv=disabled`；
⑦ 禁用 iconv 后，mpv 自己的 `-Duchardet=enabled` 因为 uchardet 硬依赖 iconv 而报错——movaslim 下一并禁用 `-Duchardet` 并去掉 `uchardet` buildInput（`mk-out-libs` 早就假设 movaslim 不需要 uchardet.dylib 于是没打包，这轮才让这个假设真正成立）。
**run 35206555833（2026-09-17）跑绿，产物只有一个 `libmpv.dylib`，10,420,552 字节 ≈ 9.94MiB**——比 movaslim 动态版（10.76MiB/13 文件）体积相近但是单文件了，跟 Android 的 6.52MiB 单文件终于是同一种可比形态。

**v3.3（同日）编译器级瘦身**：给静态依赖（dav1d/freetype/fribidi/harfbuzz/libpng）和 mpv 本体加 `-Dbuildtype=minsize -Ddebug=false -Db_ndebug=true`，ffmpeg 侧追加 `--extra-cflags`/`--extra-ldflags`（`-Os -fvisibility=hidden -ffunction-sections -fdata-sections` / `-Wl,-dead_strip`，这里是拼接进 ffmpeg 自己已解析出的 `c_args` 字符串，纯追加不覆盖）。**第一次尝试直接给这五个 meson 静态包也加 `-Dc_args="-fvisibility=hidden ..."` / `-Dc_link_args="-Wl,-dead_strip"` 翻车了**：meson 命令行的 `-Dc_args=` 是**替换**而不是追加 cross-file 里已有的 `c_args`（那里面带着 `-arch arm64 -isysroot ... -miphoneos-version-min=...` 这些必需的交叉编译标志），libpng 因此在 `cc.get_define('ZLIB_VERSION', ...)` 检测 zlib.h 时因为丢失 sysroot 报错（run 35207906272）——回退掉这两个 `-D` 参数（只保留不碰 c_args 的 buildtype/debug/b_ndebug 三项），libass 侧同理设计过的 `CFLAGS=`/`LDFLAGS=` trailing 参数（同样的覆盖风险，autoconf 的位置参数会盖掉环境变量里的 CFLAGS）也顺带撤掉，未经实测直接判定为不安全。**run 35210201885（2026-09-17）跑绿：产物 7,489,408 字节 ≈ 7.14MiB**——单是 `-Ddebug=false`/`buildtype=minsize`/`--disable-debug` 这类不碰 c_args 的选项就把 9.94MiB 压到 7.14MiB（−28%），**距 Android 的 6.52MiB 只剩约 +9.5%**，考虑到两个平台硬解 API（MediaCodec vs VideoToolbox）架构完全不同、本来就不该指望完全相等，这个差距已经相当接近。**尚未尝试**：给 5 个 meson 静态包正确地把 `-fvisibility=hidden`/`gc-sections`/`dead-strip` 追加（而非替换）进 cross-file 的 c_args（需要先从 nix 层读出 cross-file 已有的 c_args 值再拼接，比这次简单加 `-D` 麻烦，暂未做）——如果还想再往下压这最后 9.5%，这是下一个可尝试的方向。`flavors-mova-slim-ios.sh` 本体标注为已被此 patch 取代的历史记录，不再删除。checkout 未锁定 `media-kit/libmpv-darwin-build` 的 ref（用的是当时最新 main），后续应视情况锁定。**曾有 `mova-libmpv-winbuild-zhangfly` 分支走 ffmpeg n9.0+mpv 0.41（v9 线）做过 CI 验证（10.15MiB），该分支已作废不用**，不可比、不要复用。**v3.4（2026-09-17，写好未验证）**：`movaslim` flavor 此前 audio/protocol 两侧一直是"`options += audio_default_options` 原样照抄"——现在新增 `audio_movaslim_options`（decoder 只留 aac/aac_latm/mp3float/opus/flac/vorbis/pcm_s16le/pcm_s16be/pcm_s24le/pcm_s32le/pcm_f32le/pcm_u8，砍掉 DTS/TrueHD/AC3/EAC3/WMA*/APE/ATRAC*/ALS/GSM*/MPC*/RA*/Shorten/TAK/TTA/WavPack/DSD* 等一整批 mova 用不上的音频 decoder；demuxer 加 mp3/flac/ogg/wav/aac 覆盖 audioOnly 模式下的独立音频文件，砍掉 asf/avi/rm/aiff/au/wv/xwma/dsf/dts/truehd；沿用 overlay/equalizer 两个 filter，avfilter 收窄留作独立 TODO 不在这轮动）与 `protocol_movaslim_options`（只留 file/data/pipe/http/https/tcp/tls/crypto/hls 九个协议，砍掉 ftp/rtmp 全家/rtp/udp/async/cache/subfile/httpproxy——mova 是 HLS 点播/直播播放器，manifest+分片走 http(s)/tcp/tls，AES-128 解密靠 crypto，`hls` 是 ffmpeg 内部协议不是 demuxer，两者都要留），参考了 Android 侧已有的 `flavors-mova-slim.sh` 音频清单核对拼写。也调研了公开的 iOS ffmpeg 瘦身项目 FFmpegBuild（superuser404notfound）作对照，其思路是完全砍掉网络协议层改走宿主 `URLSession` 回调注入——跟 mova 直接用 ffmpeg 原生 HLS 拉流的架构不同，未采纳，仅确认了"protocol 收敛是常见且合理的瘦身方向"这一点。**改动仅落在 `libmpv-darwin-build-mova-slim.patch`，未过 CI 验证**——下次接手先跑一次 CI 确认 `git apply` 成功、meson 语法无误、ffmpeg configure 不因为音频 decoder/demuxer 名字拼错或协议依赖缺失（如 crypto 需要的加密库）而失败，编译通过后再看实际体积变化。**v3.4 CI 验证结果（2026-09-17，run 35226623492 跑绿，commit `05bce97`）**：中途踩了两个独立坑——`bf3e0e7` 手工改 unified diff 改出了损坏的 hunk（CI run 35220072144 报 `corrupt patch`），`5abe78c` 改用真实 clone+edit+`git diff` 重新生成整份 patch 修复；同时产品侧指出 mova 实际有 RTMP/RTP/UDP 直播源，不是纯 HLS，`05bce97` 把这三个协议从"砍掉"改判为"保留"（只留 ftp/async/cache/subfile/httpproxy 真正砍掉）。产物 `libmpv.dylib` **6,534,448 字节 ≈ 6.23MiB**——比 v3.3 的 7.14MiB 再降 −13%，**首次反超 Android 定稿的 6.52MiB（约 −4.4%）**。目前是 iOS 侧的当前有效记录 |
| macOS | ✅ **movaslim 三架构全绿并已接入 dist/（2026-09-18，commit `815be9e`，run `35312415642`）** | `media-kit/libmpv-darwin-build`，Nix + 固定版本 Xcode，`macos-15` runner，复用 iOS 的同一份 `libmpv-darwin-build-mova-slim.patch`（`flavor == movaslim` 全平台通用，TARGET 只换 arch 段）。v1 那次因单次编译全部 ios/iossimulator/macos 组合超时被取消（run 35167158798）的旧问题已绕开——现按 `arch: [arm64, amd64, universal]` matrix 拆分为三条独立 job 腿，`arm64` 先验证绿（run 35305553036），随后扩到 amd64/universal。三腿最终 `libmpv.dylib` 实测字节数：**arm64 7,398,112 字节（≈7.06MiB）、amd64 7,957,944 字节（≈7.59MiB）、universal 15,377,120 字节（≈14.66MiB）**——universal 是 lipo 合并 arm64+amd64 两份产物，其体积（15,377,120）与两者之和（15,356,056）基本一致（+21,064 字节，lipo 的 fat header/对齐开销），符合预期。产物已提交进 `mova/tools/ffmpeg-slim/dist/macos-{arm64,amd64,universal}/libmpv.dylib`（此前只做验证、未落 dist/；同批一并把此前一直未提交的 iOS 产物落进 `dist/ios-arm64/libmpv.dylib`，5,741,680 字节 ≈5.48MiB，来自同一 run `35312415642` 的 `ios` job）。此前 `dist/darwin/` 下两份预 movaslim 时代、default flavor 的旧 tar.gz（ios/macos universal-video-default）已删除，未来产物统一落 `macos-<arch>/` 与 `ios-arm64/` 各自目录。CI 侧新增"Commit built artifact to dist/"步骤，与 android-arm64 同款 fetch+reset+重试+`git lfs push` 逻辑，因 `darwin`/`ios` 两 job 的 dart-labs checkout 在 `path: dart-labs` 子目录（不是根目录），提交步骤加了 `working-directory: dart-labs`。 |
| Windows | ✅ **已接入 mova 工程并真机验证（2026-09-24）**：`mova/example/pubspec.yaml` 的 `dependency_overrides` 指向本地 fork 包 `media_kit_libs_windows_video_slim`，其 CMakeLists 直接引用 CI 产出的 `dist/windows-x86_64/libmpv-2.dll`，`flutter run -d windows` 实机画面+声音正常；早期"本地 MSYS2 构建链跑通，未接入/未真机"记录已过时 | 本地历史构建链改用 `media-kit/libmpv-win32-video-cmake`（`libmpv-win32-video-build` 已归档/不可用，切到其活跃后继仓库），MSYS2 ucrt64 原生 gcc 工具链（`SKIP_TOOLCHAIN=ON` 跳过从零构建交叉编译器）+ Ninja，x86_64。**2026-08-13 定稿数字：28,274,176 字节（≈26.97 MiB）**，strip 后，含 D3D11VA 硬解，`mpv`/`libmpv-2.dll`/头文件全部产出。**比 media-kit 官方发行版（35.99 MiB，同架构 x86_64）还小 −25%**，且官方版没有针对"现代主流点播+直播"做任何裁剪（完整 openssl+libssh+libsrt+完整 libplacebo/vulkan/shaderc）——这是比 Android(6.52MiB)/iOS(5.96MiB) 更合理的参照系，那两个平台架构完全不同（MediaCodec 独立硬解/VideoToolbox 系统 API），不背 GPU-next 渲染管线这种包袱，直接比"差 4-5 倍"没有意义。详见下方"Windows 瘦身构建"一节。**但 CI 工作流（`build-mova-libmpv.yml` 的 `windows` job）走的是另一条更瘦的原生 MSYS2 recipe（pacman 依赖，不用 libmpv-win32-video-cmake），2026-09-17 首次真实跑通：编译/链接/strip/dav1d 符号校验/上传 artifact 全部成功，产物 6,904,180 字节（≈6.58 MiB，比本地那份 26.97 MiB 小得多，两者不是同一套构建，不能直接比较）。唯一失败在最后"Commit built artifact to dist/"步骤**：`git push` 因为其他平台 job 已先推送而被拒（`[rejected] main -> main (fetch first)`），重试逻辑 `git fetch origin main && git rebase origin/main` 时因为工作区有未暂存改动，`git rebase` 直接报错退出（`error: cannot rebase: You have unstaged changes. Please commit or stash them.`）退出码 1。**这是 CI 脚本本身的 bug，不是 Windows 构建的问题**——android-arm64/android-other-abi(×3)/darwin/linux/windows 五个 job 共享同一套"提交本地 commit → push 被拒就 fetch+rebase 重试"逻辑，只要多个 job 并发完成、且某个 job 的工作区在 rebase 时不是纯净状态，就会复现这个失败；这次是 windows 排在最后触发的。**2026-09-17 三个连环 bug 全部修复并验证**（run 35172861864 windows job 绿）：
① "提交"逻辑改造成"每次重试先 `git fetch` + `git reset --hard origin/main` 拿到干净基线，
再在其上 `mkdir/cp/add/commit`"，不用查清楚具体是什么让工作区变脏，`reset --hard`
无论原因都能保证纯净；② 更隐蔽的一个：`actions/checkout` 不装 LFS 钩子，五个 job 原来
的 `git push` 只推了 LFS 指针提交、**从未真正上传过二进制内容**（GitHub main 一度有
6 个对象 404，已用该次 run 的 build artifact 手工比对 SHA256 后补传并验证），修复是
在 commit 后、push 前显式加一行 `git lfs push origin main`；③ windows 专属：job 全部
step 默认 `shell: msys2 {0}`，其 PATH 不含 Windows Git 自带的 `git-lfs.exe`，导致
pre-push 钩子每次都在"找不到 git-lfs"这步报错退出（现象跟①很像，根因完全不同），
修复是把"Commit built artifact to dist/"这一步单独覆盖成 `shell: bash`。三处改动都在
...
- ~~**接入 mova 工程**~~ **已完成（2026-09-24 确认，随真机验证一起落地，此前未回写）**：
  `mova/example/pubspec.yaml` 用 `dependency_overrides` 把 `media_kit_libs_windows_video`
  指到本地 fork 包 `mova/packages/media_kit_libs_windows_video_slim`。该 fork 的
  `windows/CMakeLists.txt` 跳过官方 7z 下载，直接指向 CI 产出的
  `mova/tools/ffmpeg-slim/dist/windows-x86_64/libmpv-2.dll`（本节的瘦身版），并从
  `windows-devlib/libmpv.dll.a`（用 `gendef`+`dlltool` 针对该 dll 导出表手工生成，
  dll 换版本要重新生成）+ `mpv-headers/*.h`（pin 在 build-mova-libmpv.yml windows
  job 用的 mpv commit `78d43740f52db817d98bcf24fb30a76ab6fa13ff`）拼出
  `media_kit_video` 链接期需要的目录结构，接线方法与 Android 那节"与 media_kit
  集成"思路一致（path override，不走构建时下载）。
- ~~**真机（真实 Windows 机器）播放验证**~~ **已完成（2026-09-24）**：不是这条记录的
  ...
  通过（GitHub Actions run 35948800700）。用的就是上一条"接入 mova 工程"那条链路，
  不是临时替换测试——Windows 是真实生效的接线。
