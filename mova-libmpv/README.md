...
| iOS | 🔄 **2026-09-17 v3.1 movaslim flavor patch 写好，尚未过 CI**（v3 default-flavor 首绿见 run 35194396206） | 原 `ios` job 那套从零手写的 ffmpeg+8 依赖库 meson 交叉构建 + mpv（v2 线，`flavors-mova-slim-ios.sh`，6.52MiB 那套定稿数字是本地/CI 均从未真正跑绿过的理论值）已废弃。v3 改为复用 `darwin` job 同一套上游仓库/工具链（`media-kit/libmpv-darwin-build`，Nix + 固定版本 Xcode），中间踩了两个坑（TARGET 属性名前缀取的是目录名 `mk-out-archive` 不是内部展示变量 `"archive"`；`find` 不解引用 `result/` 符号链接）后于 run 35194396206 首次全绿，用的是**upstream 默认 flavor**，产物是 `libs` 格式解出的 18 个独立 `.dylib`（libmpv.dylib 本体仅 1,890,312 字节 ≈1.80MiB，其余是 ffmpeg/dav1d/libass/freetype/harfbuzz/fribidi/mbedtls/libxml2/libpng/uchardet 各自独立的动态库）。**v3.1（同日）**：新增 `mova-libmpv/libmpv-darwin-build-mova-slim.patch`，给这套 Nix 构建加一个新 flavor `movaslim`（`nix/utils/constants/flavors.nix` + `nix/utils/default/flavors.nix` 登记，`mk-pkg-ffmpeg/meson.build` 的 `-Dflavor=movaslim` 分支），内容是把 `flavors-mova-slim-ios.sh` 里设计好但一直没接线的 DECODERS/ENCODERS/PARSERS/DEMUXERS/BSFS 清单（h264/hevc/vp9/av1 视频 + ass/srt/webvtt/movtext 字幕，砍掉 flv/h263/mpeg1/2/4/vp6/vp8/theora/msmpeg4/wmv/mjpeg 等老编解码器和 dvbsub/dvdsub/pgssub 图形字幕）移植进 meson.build 的 `video_movaslim_options`，同时把 TLS 从 mbedtls 换成 Apple 系统自带的 securetransport（去掉 `--enable-version3`，同 Linux/Windows 那两条線换 openssl/SChannel 的思路）、去掉 movaslim 专属的 libxml2（dash demuxer 不需要）与 uchardet（mpv 本来就 `-Duchardet=disabled`，纯死重）两个 dylib——改动落在 `mk-pkg-ffmpeg/{meson.build,meson.options,default.nix}` 与 `mk-out-libs/default.nix`（该文件是不管 ffmpeg 实际链接了什么、无条件把 `deps` 列表里每个包的 `.dylib` 全部拷进最终产物的地方，只在 meson 里关掉 ffmpeg 的 configure 开关并不会让对应 dylib 消失，必须同时在这里摘掉）。**已本地验证 patch 对全新 clone 干净 apply（`git apply --check` 通过）**，尚未跑一次真实 CI，产物体积待下次 run 的 step summary 实测（workflow 新增了"total bundle size"汇总行，把所有 `.dylib` 加总，这才是跟 Android 单文件 6.52MiB 可比的数字）。**架构性诚实提醒**：`libs`/`xcframeworks` 两种产物格式（读过 `mk-out-frameworks/ios/build-ios.sh`）都只是把已经各自动态链接好的 `.dylib` 逐个套壳，从未把依赖静态合并进一个 libmpv 文件——这次 patch 只砍了每个库自己的内容体积，**没有**做到 Android/旧 v2 线那种"全部依赖静态链接进单一产物"的形态，真要对齐 Android 的量级大概率还需要第二轮把 mpv+ffmpeg 依赖链改成静态链接（工作量不小，本轮先不做，视本次实测数字决定是否值得投入）。`flavors-mova-slim-ios.sh` 本体标注为已被此 patch 取代的历史记录，不再删除。checkout 未锁定 `media-kit/libmpv-darwin-build` 的 ref（用的是当时最新 main），后续应视情况锁定。**曾有 `mova-libmpv-winbuild-zhangfly` 分支走 ffmpeg n9.0+mpv 0.41（v9 线）做过 CI 验证（10.15MiB），该分支已作废不用**，不可比、不要复用 |
