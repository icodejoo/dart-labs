...
| macOS | ⏸️ **暂停（2026-09-17，iOS 专项迭代期间 `if: false`）** | `media-kit/libmpv-darwin-build`，Nix + 固定版本 Xcode，`macos-15` runner。v1 直接用上游自带的"video/default" flavor，未照搬瘦身清单。**2026-09-17 CI run 35167158798**：编译正常推进（28 分钟时仍在给 `iossimulator-arm64` 编译 ffmpeg 众多子 target 之一），因为等不及被 `gh run cancel` 主动取消，日志显示 `The operation was canceled`，不是构建失败。这个 job 要一次性编译 ios/iossimulator/macos 的全部 arch×variant×flavor 组合，天然比其他平台（3-9 分钟收工）慢得多——待 iOS 专项验证告一段落后重新 `if: true`，给够时间（预计 45-90+ 分钟）或考虑拆分/精简 flavor |
