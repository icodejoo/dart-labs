// Entry point for the fsvg-vs-flutter_svg 1000-icon benchmark. The library
...
//
// fsvg 对比 flutter_svg 的千图标基准入口。被测库与滚动轮次数通过 --dart-define
...
const _libName = String.fromEnvironment('LIB', defaultValue: 'fsvg');
...
Future<void> main() async {
  ...
  // FSvgStatic silently fall back to a blank SizedBox on every render (its
  // errorBuilder is null by default), which would otherwise make this
  // benchmark measure "building empty boxes" instead of real fsvg work.
  //
  // 首次调用 parseSvg() 前必须初始化一次；漏掉这步会导致 FSvgStatic 每次渲染都
  ...
  if (_libName == 'compare') {
    // Single-build sequential mode: runs fsvg static -> flutter_svg static ->
    ...
    //
    // 单编译顺序模式：在这一个进程内依次跑完 fsvg 静态 -> flutter_svg 静态 ->
  ...
  final lib = _libName == 'flutter_svg' ? BenchLib.flutterSvg : BenchLib.fsvg;
  runApp(MaterialApp(
