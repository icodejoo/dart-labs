...
// timing, parse timing (fsvg only) and RSS memory samples throughout.
//
// 基准测试页：网格渲染 1000 个不同的图标 SVG，驱动来回滚动（顶部/底部弹跳），
// 并全程采集帧耗时、解析耗时（仅 fsvg）与 RSS 内存样本。
...
/// Example:
/// ```dart
/// BenchRunner(lib: BenchLib.fsvg, cycles: 6, itemCount: 1000,
...
class _BenchRunnerState extends State<BenchRunner> {
  ...
  void _printReport() {
    ...
    final buf = StringBuffer()
      ...
      ..writeln('parse : $parse (fsvg only; flutter_svg has no equivalent public hook)')
      ...
      body: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: widget.lib == BenchLib.fsvg
                ? FSvgStatic(source, width: 32, height: 32)
