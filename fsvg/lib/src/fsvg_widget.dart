...
/// [FSvgAnimated.string] (this project's original SMIL engine); otherwise it
/// renders through [FSvgStatic] (Rust `usvg` parser → cached [ui.Picture]).
...
/// `<set>`）动画标记，走 [FSvgAnimated.string]（本项目原创 SMIL 引擎）；否则走
/// [FSvgStatic]（Rust `usvg` 解析器 → 缓存的 [ui.Picture]）。CSS
/// `@keyframes`/`animation-*` 动画暂不检测/支持——见
/// `lib/src/animation/animation_detector.dart`。
///
/// Example:
/// ```dart
/// FSvg.string(svgSource, width: 48, height: 48)
/// ```
