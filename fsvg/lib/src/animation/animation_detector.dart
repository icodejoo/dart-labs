...
// at all, so [FSvg] can route plain icons to the faster Rust static path.
//
// 廉价的语法级嗅探，判断 SVG 源是否需要动画引擎，供 [FSvg] 把普通图标路由到
// 更快的 Rust 静态路径。
...
class AnimationDetector {
  ...
  // detection — see fsvg CLAUDE.md's CSS section).
  //
  ...
  // 针对本引擎更小的支持标签集（不含 CSS 检测——见 fsvg CLAUDE.md 的 CSS
  // 章节）重新写的。
