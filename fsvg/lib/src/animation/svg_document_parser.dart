...
// normal pubspec dependency, not vendored code — see fsvg CLAUDE.md). Also
...
// and the `FSvg` dispatch in `fsvg_widget.dart` only ever see [SvgDocument]/
...
// `animated_svg_widget.dart`，以及 `fsvg_widget.dart` 里的 `FSvg` 分发）之间
...
/// themselves animated (only their *content* can be — see fsvg CLAUDE.md
/// task notes).
///
...
/// 动画切换（只有其*内容*可能被动画——见 fsvg CLAUDE.md 任务记录）。
