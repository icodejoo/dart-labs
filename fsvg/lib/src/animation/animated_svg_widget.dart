...
// vendored F `AnimatedSvgPicture` — see fsvg CLAUDE.md. Original
// implementation.
//
// 公开的动画 SVG 组件：只解析一次源串，之后用 [AnimationController] 驱动，
// 在每帧对时间线采样并通过 [AnimatedSvgPainter] 重绘。这是替代 vendor 的 F
// 中 `AnimatedSvgPicture` 的原创实现——见 fsvg CLAUDE.md。原创实现。
...
///
/// See fsvg CLAUDE.md for the supported SMIL subset: `<animate>` on
...
/// Example:
/// ```dart
/// FSvgAnimated.string(svgSource, width: 48, height: 48)
/// ```
///
/// 用本项目原创动画引擎（解析 → 时间线 → 逐帧采样 → 绘制）渲染 SMIL 动画
/// SVG 字符串。
///
/// 支持的 SMIL 子集见 fsvg CLAUDE.md：作用于 `stroke-dashoffset`（泛化后可作用
...
/// FSvgAnimated.string(svgSource, width: 48, height: 48)
/// ```
class FSvgAnimated extends StatefulWidget {
  /// Creates the animated renderer from a raw SVG string.
  ///
  /// 用原始 SVG 字符串创建动画渲染组件。
  const FSvgAnimated.string(
    this.source, {
    super.key,
    this.width,
    this.height,
  ...
  @override
  State<FSvgAnimated> createState() => _FSvgAnimatedState();
}
...
class _FSvgAnimatedState extends State<FSvgAnimated> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(FSvgAnimated oldWidget) {
