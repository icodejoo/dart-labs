...
class RustSvgPictureCache {
  ...
  /// recursion) — see fsvg CLAUDE.md for the documented limitation.
  ///
  /// 绘制单个 [SvgPath]，并应用其继承的裁剪区域、遮罩与高斯模糊。
  ///
  /// 绘制遮罩内容或图案贴片内容时置 [nested]：这些子显示列表按朴素方式绘制
  /// （不再递归处理裁剪/遮罩/模糊/图案）——限制说明见 fsvg CLAUDE.md。
...
/// keyframes — see [FSvg] for the dispatch widget that picks this vs the
...
/// keyframes 的 SVG——需要自动判定走哪条渲染路径请用 [FSvg]。
///
/// Example:
/// ```dart
/// FSvgStatic('<svg ...>...</svg>', width: 48, height: 48)
/// ```
class FSvgStatic extends StatelessWidget {
  ...
  /// [FSvgAnimated]'s `theme` param for API consistency across the animated
  /// and static paths.
  ///
  /// 控制 `currentColor` 的主题；默认不透明黑色。与 [FSvgAnimated] 的 `theme`
