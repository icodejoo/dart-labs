...
pub fn parse_svg(data: String, current_color: Option<u32>) -> Result<SvgScene, String> {
    ...
    // `<text>` needs real font outlines, and building a system font database
    // costs ~100ms, so only pay it for sources that actually contain text —
    // and only once per process (see [system_fontdb]).
    //
    // `<text>` 需要真实的字形轮廓，而构建系统字体库约需 100ms，因此只有真正
    // 含文本的源才付这笔开销，且每个进程只付一次（见 [system_fontdb]）。
    if data.contains("<text") {
        opt.fontdb = system_fontdb();
    }
...
/// Process-wide, lazily built system font database, shared by every text-
/// bearing parse.
///
/// `fontdb::Database::load_system_fonts` walks the OS font directories and
/// costs on the order of 100ms, which would be unacceptable per call; the
/// `OnceLock` makes it a one-time cost paid only when a source actually
/// contains `<text>`.
///
/// 进程级、按需构建的系统字体库，供所有含文本的解析共用。
///
/// `fontdb::Database::load_system_fonts` 要遍历操作系统字体目录，开销在 100ms
/// 量级，逐次调用不可接受；用 `OnceLock` 使其只在源中确实含 `<text>` 时付一次。
...
fn collect(group: &usvg::Group, ctx: &Inherited, out: &mut Vec<SvgPath>, images: &mut Vec<SvgImage>) {
    for node in group.children() {
        match node {
            ...
            // `<text>` is rendered through usvg's own best-effort outline
            // flattening — the same conversion `usvg`'s CLI performs — so it
            // reuses the plain path pipeline with no text-specific model.
            //
            // `<text>` 走 usvg 自身尽力而为的轮廓展平（与 `usvg` CLI 的转换
            // 相同），因此直接复用普通路径管线，无需专门的文本模型。
            usvg::Node::Text(t) => collect(t.flattened(), ctx, out, images),
        }
    }
}
...
/// fsvg CLAUDE.md for the investigation) is combined via
...
mod tests {
    ...
    #[test]
    fn text_is_flattened_into_glyph_outlines() {
        // Relies on real system fonts, so it asserts only what is stable
        // across machines: some glyph geometry exists, it is filled with the
        // requested colour, and it sits roughly where the text was anchored.
        //
        // 依赖真实系统字体，因此只断言跨机器稳定的部分：确实产生了字形几何、
        // 填充色为指定颜色、且大致落在文本锚点附近。
        ...
        assert!(
            ...
            "<text> must flatten into glyph outlines, not vanish"
        );
        assert!(scene.paths.iter().any(|p| p.fill_argb == 0xFFFF0000));
        let min_x = scene
            .paths
            .iter()
            .flat_map(|p| p.points.iter().step_by(2))
            .copied()
            .fold(f32::INFINITY, f32::min);
        assert!(
            (5.0..60.0).contains(&min_x),
            "glyphs should start near the text anchor x=10, got {min_x}"
        );
    }
