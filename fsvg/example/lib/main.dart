...
// Rust static path's new `currentColor` substitution via `FSvgStatic`'s
...
// features (see fsvg CLAUDE.md "当前动画引擎支持" / static-path notes).
...
/// Remounts an [FSvg.string] on a timer so a non-repeating (no
/// `repeatCount="indefinite"`) demo animation replays instead of staying
/// frozen at its final frame once played.
///
/// 定时重新挂载 [FSvg.string]，让不带 `repeatCount="indefinite"` 的演示动画
/// 反复重播，而不是播放一次后永远定格在终态。
class _RestartingSvg extends StatefulWidget {
  const _RestartingSvg(this.source, {required this.width, required this.height, required this.interval});
...
class _RestartingSvgState extends State<_RestartingSvg> {
  ...
  void initState() {
    super.initState();
    _timer = Timer.periodic(widget.interval, (_) => setState(() => _generation++));
  }
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: const Text('fsvg acceptance test')),
        body: Center(
          child: SingleChildScrollView(
            ...
            children: [
              ...
              const SizedBox(height: 48),
              const Text('Rotation marker (unambiguous animateTransform proof)'),
              const SizedBox(height: 8),
              FSvg.string(
                kRotationMarkerSvg,
                width: 96,
              ...
              const SizedBox(height: 48),
              const Text('paint-order: default (fill then stroke) vs stroke-first'),
              const SizedBox(height: 8),
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  FSvg.string(_paintOrderCircleSvg(strokeFirst: false), width: 96, height: 96),
                  const SizedBox(width: 32),
                  FSvg.string(_paintOrderCircleSvg(strokeFirst: true), width: 96, height: 96),
                ],
              ),
              const SizedBox(height: 48),
              const Text('Embedded <image> (base64 PNG)'),
              const SizedBox(height: 8),
              FSvg.string(kEmbeddedImageSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('Named CSS colors (fill="tomato" stroke="cornflowerblue")'),
              const SizedBox(height: 8),
              FSvg.string(kNamedColorsSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('Static <g transform="translate() rotate() scale()">'),
              const SizedBox(height: 8),
              FSvg.string(kGroupTransformSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('animateTransform type="skewX"'),
              const SizedBox(height: 8),
              FSvg.string(kSkewXSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('<use> of a <defs> shape with its own <animate>'),
              const SizedBox(height: 8),
              FSvg.string(kUseAnimatedSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              ...
              const SizedBox(height: 8),
              FSvg.string(kAnimateMotionSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('Static linearGradient fill'),
              const SizedBox(height: 8),
              FSvg.string(kLinearGradientSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('Animated <mask> (Task 1)'),
              const SizedBox(height: 8),
              FSvg.string(kAnimatedMaskSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('<text> with animated opacity (Task 2)'),
              const SizedBox(height: 8),
              FSvg.string(kTextSvg, width: 160, height: 48),
              const SizedBox(height: 48),
              const Text('<animateMotion keyPoints/keyTimes> (Task 3)'),
              const SizedBox(height: 8),
              FSvg.string(kAnimateMotionKeyPointsSvg, width: 96, height: 96),
              const SizedBox(height: 48),
              const Text('Animated gradient: geometry + stop-color (Task 4)'),
              const SizedBox(height: 8),
              FSvg.string(kAnimatedGradientSvg, width: 96, height: 96),
            ],
            ),
          ),
        ),
