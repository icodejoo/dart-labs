...
//
// The colour/transform cases depend on the native library (`fsvg.dll`/`.so`),
...
//
// 颜色/变换用例依赖原生库（`fsvg.dll`/`.so`），而纯 `flutter test` 的 host VM
// 没有产出它的构建步骤，此时用例自行跳过（与
// `test/rust_image_smoke_test.dart` 同一约定）并打印原因，避免把"跳过"误当成
// "通过"。

import 'package:flutter_test/flutter_test.dart';
import 'package:fsvg/fsvg.dart' show RustLib;
